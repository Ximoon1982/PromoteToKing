#!/usr/bin/env bash
# Promote to King v2.9.0 - argumentless IONOS CRON installer.
...
DISPATCHER="$SITE_ROOT/cron-dispatch-v2.9.0.sh"
...
if [[ -n "$PROBLEM" ]]; then
  ...
  "$PHP_BIN" -r '
    ...
    $j=is_file($shared)?json_decode((string)file_get_contents($shared),true):[];if(!is_array($j))$j=[];
    $existing=trim((string)($j["cronToken"]??""));
    if($existing===""||str_starts_with($existing,"CHANGE_")){$j["cronToken"]=$token;echo "Shared CRON token: repaired from protected Team Points configuration.\n";}else{echo "Shared CRON token: already configured.\n";}
    $j["schemaVersion"]=(int)($j["schemaVersion"]??1);
    if(trim((string)($j["trafficAnalyticsSecret"]??""))===""){$j["trafficAnalyticsSecret"]=bin2hex(random_bytes(32));echo "Traffic analytics secret: generated.\n";}else{echo "Traffic analytics secret: already configured.\n";}
  ...
  ' "$TP_CONFIG" "$SHARED_CONFIG" || TOKEN_RC=$?
  ...
  if [[ "$TOKEN_RC" -ne 0 ]]; then
    ...
    {
      printf '%s\n' "$KEEP"
      printf '%s\n' 'SHELL=/bin/bash' 'PATH=/usr/local/bin:/usr/bin:/bin' '' '# BEGIN PROMOTE TO KING v2.9.0'
      ...
      printf '%s\n' '# END PROMOTE TO KING v2.9.0'
    } > "$TMP"
    if $CRONTAB_BIN "$TMP"; then
      INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
      COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.0.sh' || true)
      if [[ "$COUNT" -eq 4 ]]; then
        echo "SUCCESS: v2.9.0 CRON schedule installed (4 tasks)."
        printf '%s\n' "$INSTALLED"
      else
        echo "ERROR: verification found $COUNT v2.9.0 dispatcher entries; restoring previous crontab." >&2
