...
HTTP_STATUS=$($CURL_BIN --silent --show-error --location --connect-timeout 10 --max-time 55 --user-agent 'PromoteToKing-Cron/2.9.1' --config "$CURL_CONFIG" --output "$TMP" --write-out '%{http_code}' "$URL" 2>>"$LOG")
CURL_EXIT=$?
