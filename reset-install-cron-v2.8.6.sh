...
usage() {
  cat <<'USAGE'
Usage:
  ./reset-install-cron-v2.8.6.sh TEAM_POINTS_CRON_TOKEN SHARED_CRON_TOKEN [BASE_URL]

Example:
  ./reset-install-cron-v2.8.6.sh 'team-token-here' 'shared-token-here' 'https://www.promotetoking.org'

WARNING: this script deliberately deletes the ENTIRE crontab of the current SSH user,
then installs only the Promote to King v2.8.6 entries below.
USAGE
}
...
echo "Installed Promote to King v2.8.6 crontab for user: $(id -un)"
echo
