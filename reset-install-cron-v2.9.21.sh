#!/usr/bin/env bash
# Promote to King v2.9.21 - argumentless IONOS CRON installer.
...
DISPATCHER="$SITE_ROOT/cron-dispatch-v2.9.21.sh"
WEEKLY_BACKUP="$SITE_ROOT/weekly-backup-v2.9.21.sh"
MIAC_INSTALLER="$SITE_ROOT/install-miac-seed-v2.9.21.sh"
...
[[ ! -f "$WEEKLY_BACKUP" ]] && PROBLEM="Missing weekly backup script: $WEEKLY_BACKUP"
[[ ! -f "$MIAC_INSTALLER" ]] && PROBLEM="Missing MIAC seed installer: $MIAC_INSTALLER"
...
if [[ -n "$PROBLEM" ]]; then
  ...
  if [[ "$TOKEN_RC" -ne 0 ]]; then
    ...
    chmod 755 "$DISPATCHER" "$WEEKLY_BACKUP" "$MIAC_INSTALLER" 2>/dev/null || true
    if ! P2K_SITE_ROOT="$SITE_ROOT" "$BASH_BIN" "$MIAC_INSTALLER"; then echo "ERROR: MIAC seed installation failed; crontab was not modified." >&2; exit 9; fi
    ...
    {
      printf '%s\n' "$KEEP"
      # v2.9.21: do not duplicate the exact environment lines installed by earlier P2K CRON installers.
      printf '%s\n' "$KEEP" | grep -qx 'SHELL=/bin/bash' || printf '%s\n' 'SHELL=/bin/bash'
      printf '%s\n' "$KEEP" | grep -qx 'PATH=/usr/local/bin:/usr/bin:/bin' || printf '%s\n' 'PATH=/usr/local/bin:/usr/bin:/bin'
      printf '%s\n' '' '# BEGIN PROMOTE TO KING v2.9.21'
      ...
      printf '%s\n' "37 3 * * 0 P2K_SITE_ROOT=$SITE_ROOT $BASH_BIN $WEEKLY_BACKUP"
      printf '%s\n' '# END PROMOTE TO KING v2.9.21'
    } > "$TMP"
    if $CRONTAB_BIN "$TMP"; then
      INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
      COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.21.sh' || true)
      WEEKLY_COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'weekly-backup-v2.9.21.sh' || true)
      if [[ "$COUNT" -eq 4 && "$WEEKLY_COUNT" -eq 1 ]]; then
        echo "SUCCESS: v2.9.21 CRON schedule installed (4 operational tasks + 1 weekly long-life backup)."
        printf '%s\n' "$INSTALLED"
      else
        echo "ERROR: verification found $COUNT dispatcher and $WEEKLY_COUNT weekly-backup entries; restoring previous crontab." >&2
