#!/usr/bin/env bash
# Promote to King v2.9.22.2 -> v2.9.22.3 verified Seniority artwork updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.2_to_v2.9.22.3_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.2_to_v2.9.22.3.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.2-to-v2.9.22.3.tar.gz"
RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.2-to-v2.9.22.3.tar.gz"
STAGE="$SITE_ROOT/.p2k-v29223-stage-$STAMP"
WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v29223-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.3" ]]; then echo "Promote to King is already VERSION 2.9.22.3; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.2" ]]&&PROBLEM="Expected production VERSION 2.9.22.2, found '$CURRENT'."
...
echo "===== P2K v2.9.22.2 -> v2.9.22.3 SENIORITY ARTWORK UPDATE ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.2"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.3"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.2-to-v2.9.22.3" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.2 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.3" ]]||POST_OK=0
grep -q 'version: "2.9.22.3"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.3"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  for key in seniority-1m seniority-3m seniority-6m seniority-1y seniority-2y seniority-3y seniority-5y; do
    [[ -f "$SITE_ROOT/assets/images/achievements/$key.png" && -f "$SITE_ROOT/assets/images/achievements/thumbs/128/$key.webp" ]] || POST_OK=0
    grep -q "assets/images/achievements/$key.png" "$SITE_ROOT/server/team-points/src/AchievementCatalog.php" || POST_OK=0
  done
fi
if [[ "$POST_OK" -eq 1 ]]; then node_bin=$(command -v node||true); [[ -n "$node_bin" ]]&&for f in assets/js/shared/api-client.js assets/js/site-config.js; do "$node_bin" --check "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
...
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.3 Seniority artwork update installed."; echo "Core 15 / Analytics 7 / existing v2.9.22 CRON contract retained unchanged."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
rollback; echo "ERROR: v2.9.22.3 validation failed; release files were rolled back." >&2; exit 10
