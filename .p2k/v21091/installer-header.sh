#!/usr/bin/env bash
# Promote to King v2.10.9.1 incremental corrective installer - R3 packaging.
# Production-safe boundary: VERSION-based only; no repository/test/source-manifest assumptions.
set -Eeuo pipefail

TARGET_VERSION="2.10.9.1"
PREDECESSOR_VERSION="2.10.9"
TARGET="${1:-${P2K_SITE_ROOT:-$PWD}}"
MARKER="__P2K_V21091_R3_PAYLOAD_BELOW__"
SELF="$0"
MANIFEST=".p2k-v21091-r3-payload-manifest.json"
EXPECTED_FILES="35"
...
case "$CURRENT" in
  ...
  *) fail "Unsupported target VERSION '$CURRENT'. Expected $PREDECESSOR_VERSION (or $TARGET_VERSION for replay)." ;;
esac

STAGE="$(mktemp -d "${TMPDIR:-/tmp}/p2k-v21091-r3.XXXXXX")"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
BACKUP="$TARGET/.p2k-backups/v2.10.9.1-r3-$STAMP-$$"
...
on_error(){
  ...
  if [[ "$ROLLBACK_ARMED" == 1 ]]; then
    echo "Installation failed; restoring touched files from $BACKUP ..." >&2
    restore_files || true
  fi
  cleanup
  exit "$rc"
...
if m.get('version')!='2.10.9.1' or m.get('predecessor')!='2.10.9': raise SystemExit('manifest version boundary mismatch')
...
# Deliberately do NOT require repository-only files, tests, source manifests, or predecessor hashes.
# VERSION is the deployment boundary; all touched files are backed up before replacement.
[[ "$CURRENT" == "$PREDECESSOR_VERSION" || "$CURRENT" == "$TARGET_VERSION" ]] || fail "version boundary changed unexpectedly"
for f in server/team-points/src/McaIndexParser.php server/team-points/src/McaArenaParser.php server/team-points/src/McaResultsCronService.php server/team-points/bin/mca-results-sync.php; do
  "$PHP_BIN" -d display_errors=1 -l "$STAGE/$f" >/dev/null
  echo "syntax OK: $f"
done
python3 -m json.tool "$STAGE/site-manifest.json" >/dev/null
python3 -m json.tool "$STAGE/RELEASE_v2.10.9.1.json" >/dev/null
...
for row in m['files']:
    ...
    tmp=dst.with_name(dst.name+f'.p2k-v21091-r3-{os.getpid()}.tmp')
    shutil.copy2(src,tmp); os.replace(tmp,dst)
PYINSTALL
if [[ "${P2K_INSTALL_TEST_FAIL_AFTER_COPY:-0}" == 1 ]]; then
  echo "TEST: forcing failure after file installation." >&2
  false
fi
...
for f in server/team-points/src/McaIndexParser.php server/team-points/src/McaArenaParser.php server/team-points/src/McaResultsCronService.php server/team-points/bin/mca-results-sync.php; do
  "$PHP_BIN" -d display_errors=1 -l "$TARGET/$f" >/dev/null
done
ROLLBACK_ARMED=0

echo "Promote to King v$TARGET_VERSION installed successfully."
echo "Backup retained at: $BACKUP"
echo "No database schema or CRON changes were made. Existing runtime data was preserved."
echo "Legacy MCA date_index failures will be requeued automatically by the existing MCA worker."
exit 0

__P2K_V21091_R3_PAYLOAD_BELOW__
