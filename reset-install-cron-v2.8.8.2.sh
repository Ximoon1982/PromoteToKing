...
if [[ $# -ne 0 ]]; then
  echo "This v2.8.8.2 helper takes no arguments." >&2
  exit 2
fi
...
trap 'rm -f "$TMP"' EXIT
cat > "$TMP" <<EOF_CRON
# BEGIN PROMOTE TO KING v2.8.8.2
...
echo "Installed Promote to King v2.8.8.2 crontab for user: $(id -un)"
