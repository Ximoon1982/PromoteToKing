#!/usr/bin/env bash
# Promote to King v2.9.8 - argumentless IONOS CRON installer.
...
DISPATCHER="$SITE_ROOT/cron-dispatch-v2.9.8.sh"
...
if [[ -n "$PROBLEM" ]]; then
  ...
  if [[ "$TOKEN_RC" -ne 0 ]]; then
    ...
    {
      printf '%s\n' "$KEEP"
      printf '%s\n' 'SHELL=/bin/bash' 'PATH=/usr/local/bin:/usr/bin:/bin' '' '# BEGIN PROMOTE TO KING v2.9.8'
      ...
      printf '%s\n' '# END PROMOTE TO KING v2.9.8'
    } > "$TMP"
    if $CRONTAB_BIN "$TMP"; then
      INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
      COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.8.sh' || true)
      if [[ "$COUNT" -eq 4 ]]; then
        echo "SUCCESS: v2.9.8 CRON schedule installed (4 tasks)."
        printf '%s\n' "$INSTALLED"
      else
        echo "ERROR: verification found $COUNT v2.9.8 dispatcher entries; restoring previous crontab." >&2
