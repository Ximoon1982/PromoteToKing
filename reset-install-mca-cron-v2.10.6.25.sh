#!/usr/bin/env bash
# Promote to King v2.10.6.25 — MCA Results synchronization twice-daily scheduler.
set -Eeuo pipefail
ROOT="${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)}"
ROOT="$(cd "$ROOT" && pwd -P)"
RUNNER="$ROOT/cron-mca-results-v2.10.6.25.sh"
...
for cand in "${P2K_PHP_CLI:-}" /usr/bin/php8.5-cli /usr/bin/php8.5 /usr/bin/php8.4-cli /usr/bin/php8.4 /usr/bin/php8.3-cli /usr/bin/php8.3 /usr/bin/php8.2-cli /usr/bin/php8.2 /usr/bin/php8.1-cli /usr/bin/php8.1 /usr/bin/php8.0-cli /usr/bin/php8.0 php8.5-cli php8.5 php8.4-cli php8.4 php8.3-cli php8.3 php8.2-cli php8.2 php8.1-cli php8.1 php8.0-cli php8.0 php; do
  ...
  if "$resolved" -r 'exit(PHP_SAPI === "cli" && PHP_VERSION_ID >= 80100 ? 0 : 1);' >/dev/null 2>&1; then PHP_BIN="$resolved"; break; fi
done
[[ -n "$PHP_BIN" ]] || { echo "ERROR: Compatible PHP CLI not found (PHP 8.1+ required). On IONOS use /usr/bin/php8.5-cli or /usr/bin/php8.5." >&2; exit 2; }
...
printf '%s\n' "$OLD" | awk '
  ...
  !skip && $0 !~ /cron-mca-results-v2\.10\.6(\.25)?\.sh/ {print}
' > "$TMP"
while [[ -s "$TMP" && -z "$(tail -n1 "$TMP")" ]]; do sed -i '$d' "$TMP"; done
[[ ! -s "$TMP" ]] || printf '\n' >> "$TMP"
cat >> "$TMP" <<EOF
# P2K_MCA_RESULTS_SYNC_BEGIN
# MCA Results synchronization v2.10.6.25 — twice daily; discovery + missing Results CSV acquisition only, serialized server-side at >=1 second spacing. Historical date backfill is manual-only.
17 0,12 * * * P2K_SITE_ROOT=$ROOT P2K_PHP_CLI=$PHP_BIN $BASH_BIN $RUNNER >/dev/null 2>&1
# P2K_MCA_RESULTS_SYNC_END
EOF
crontab "$TMP"
COUNT="$(crontab -l 2>/dev/null | grep -c 'cron-mca-results-v2.10.6.25.sh' || true)"
if [[ "$COUNT" != "1" ]]; then
...
echo "MCA Results synchronization scheduler installed: twice daily at minute 17 of hours 00 and 12 (server time)."
