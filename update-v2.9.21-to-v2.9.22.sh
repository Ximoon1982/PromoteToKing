#!/usr/bin/env bash
# Promote to King v2.9.21 -> v2.9.22 verified incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.21_to_v2.9.22_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.21_to_v2.9.22.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.21-to-v2.9.22.tar.gz"
RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.21-to-v2.9.22.tar.gz"
STAGE="$SITE_ROOT/.p2k-v2922-stage-$STAMP"
WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v2922-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22" ]]; then
  echo "Promote to King is already VERSION 2.9.22; no files were changed."
  rm -rf "$WORK_TMP" 2>/dev/null || true
  exit 0
fi
[[ "$CURRENT" != "2.9.21" ]] && PROBLEM="Expected verified production VERSION 2.9.21, found '$CURRENT'."
...
printf '%s\n' "===== P2K v2.9.21 -> v2.9.22 VERIFIED INCREMENTAL UPDATE =====" "Root:    $SITE_ROOT" "Archive: $ARCHIVE" "Backup:  compact archives under $BACKUP_ROOT"
...
"$PHP_BIN" -r '
  ...
  if(!is_array($m)||($m["from"]??"")!=="2.9.21"||($m["to"]??"")!=="2.9.22"||!is_array($m["files"]??null)||!is_array($m["removed"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.21-to-v2.9.22" || exit 6
...
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22" ]] || POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then
  grep -q 'version: "2.9.22"' "$SITE_ROOT/assets/js/site-config.js" || POST_OK=0
  grep -q '"version": "2.9.22"' "$SITE_ROOT/site-manifest.json" || POST_OK=0
  grep -q 'CORE_SCHEMA_VERSION = 15' "$SITE_ROOT/server/team-points/src/Repository.php" || POST_OK=0
fi

if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== VERIFY CORE 15 / ANALYTICS 7 ====="
  "$PHP_BIN" -r '
    ...
    if(!$repo->schemaInstalled()||$repo->schemaVersion()<15){if(!$repo->upgradeExistingSchema()){fwrite(STDERR,"Schema recovery failed.\n");exit(2);}}
    if($repo->schemaVersion()<15||$repo->analyticsSchemaVersion()<7){fwrite(STDERR,"Schema validation failed.\n");exit(3);}
...
if [[ "$POST_OK" -eq 1 ]]; then
  for f in "$SITE_ROOT/cron-dispatch-v2.9.22.sh" "$SITE_ROOT/reset-install-cron-v2.9.22.sh" "$SITE_ROOT/install-oauth-v2.9.22.sh" "$SITE_ROOT/install-miac-seed-v2.9.22.sh" "$SITE_ROOT/weekly-backup-v2.9.22.sh" "$SITE_ROOT/tools/release/p2k-state-backup.sh" "$SITE_ROOT/tools/release/p2k-state-restore.sh"; do [[ -f "$f" ]] && bash -n "$f" || { POST_OK=0; break; }; done
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== INSTALL / VERIFY v2.9.22 CRON ====="
  P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" P2K_PRESERVE_SHARED_CONFIG=1 "$SITE_ROOT/reset-install-cron-v2.9.22.sh" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 && -s "$PROTECTED_HASHES" ]]; then
...
if [[ "$POST_OK" -eq 1 ]]; then
  INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true); COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.22.sh' || true); WEEKLY_COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'weekly-backup-v2.9.22.sh' || true)
  [[ "$COUNT" -eq 4 && "$WEEKLY_COUNT" -eq 1 ]] || { echo "CRON verification found $COUNT operational and $WEEKLY_COUNT weekly v2.9.22 entries; expected 4 + 1." >&2; POST_OK=0; }
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  ...
  echo "SUCCESS: Promote to King v2.9.22 installed from verified v2.9.21."
...
rollback_update
echo "ERROR: v2.9.22 validation failed; release files, mutable state and CRON were rolled back." >&2
exit 10
