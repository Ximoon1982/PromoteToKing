#!/usr/bin/env bash
# Promote to King v2.9.12 -> v2.9.13 verified incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.12_to_v2.9.13_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.12_to_v2.9.13.json"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$SITE_ROOT/_backup/${STAMP}_v2.9.12_to_v2.9.13"
STAGE="$SITE_ROOT/.p2k-v2913-stage-$STAMP"
...
if [[ "$CURRENT" == "2.9.13" ]]; then
  echo "Promote to King is already VERSION 2.9.13; no files were changed."
  exit 0
fi
[[ "$CURRENT" != "2.9.12" ]] && PROBLEM="Expected verified production VERSION 2.9.12, found '$CURRENT'."
...
echo "===== P2K v2.9.12 -> v2.9.13 VERIFIED INCREMENTAL UPDATE ====="
echo "Root:    $SITE_ROOT"
...
"$PHP_BIN" -r '
  ...
  if(!is_array($m)||($m["from"]??"")!=="2.9.12"||($m["to"]??"")!=="2.9.13"||!is_array($m["files"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
...
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.13" ]] || POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then
  grep -q 'version: "2.9.13"' "$SITE_ROOT/assets/js/site-config.js" || POST_OK=0
  grep -q '"version": "2.9.13"' "$SITE_ROOT/site-manifest.json" || POST_OK=0
  grep -q 'CORE_SCHEMA_VERSION = 13' "$SITE_ROOT/server/team-points/src/Repository.php" || POST_OK=0
fi

if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== MIGRATE / COMPACT CORE 13 QUEUE SCHEMA ====="
  "$PHP_BIN" -r '
    require $argv[1]."/server/team-points/src/bootstrap.php";
    $core=\P2K\TeamPoints\Database::core();$analytics=\P2K\TeamPoints\Database::analytics();
    $repo=new \P2K\TeamPoints\Repository($core,$analytics);
    if(!$repo->schemaInstalled()||$repo->schemaVersion()<13){if(!$repo->upgradeExistingSchema()){fwrite(STDERR,"Schema upgrade failed.\n");exit(2);}}
    if($repo->schemaVersion()<13||$repo->analyticsSchemaVersion()<6){fwrite(STDERR,"Schema validation failed.\n");exit(3);}
    $remaining=$repo->legacyActiveQueueCount();
    if($remaining>0){$result=$repo->compactOutstandingQueue(null,1000000);echo "Queue compaction: ".json_encode($result,JSON_UNESCAPED_SLASHES)."\n";$remaining=$repo->legacyActiveQueueCount();}
    if($remaining>0){fwrite(STDERR,"Queue compaction left ".$remaining." legacy active row(s).\n");exit(4);}
...
if [[ "$POST_OK" -eq 1 ]]; then
  for f in "$SITE_ROOT/cron-dispatch-v2.9.13.sh" "$SITE_ROOT/reset-install-cron-v2.9.13.sh" "$SITE_ROOT/install-oauth-v2.9.13.sh"; do
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== INSTALL / VERIFY v2.9.13 CRON ====="
  P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" P2K_PRESERVE_SHARED_CONFIG=1 "$SITE_ROOT/reset-install-cron-v2.9.13.sh" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then
  for rel in server/team-points/config/config.local.php server/team-points/config/oauth.local.php data/server-config.json; do
    before="$BACKUP/$rel"; after="$SITE_ROOT/$rel"
    if [[ -f "$before" ]]; then cmp -s "$before" "$after" || { echo "Protected file changed during CRON install: $rel" >&2; POST_OK=0; break; }; fi
  done
fi
if [[ "$POST_OK" -eq 1 ]]; then
  INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
  COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.13.sh' || true)
  [[ "$COUNT" -eq 4 ]] || { echo "CRON verification found $COUNT v2.9.13 entries, expected 4." >&2; POST_OK=0; }
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "SUCCESS: Promote to King v2.9.13 installed from verified v2.9.12."
  echo "Backup: $BACKUP"
  echo "Protected OAuth configuration (if present) was preserved."
  exit 0
fi

rollback_update
echo "ERROR: v2.9.13 validation failed; changed release files and CRON state were rolled back." >&2
exit 10
