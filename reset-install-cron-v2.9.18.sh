#!/usr/bin/env bash
# Promote to King v2.9.18 - argumentless IONOS CRON installer.
...
DISPATCHER="$SITE_ROOT/cron-dispatch-v2.9.18.sh"
WEEKLY_BACKUP="$SITE_ROOT/weekly-backup-v2.9.18.sh"
MIAC_INSTALLER="$SITE_ROOT/install-miac-seed-v2.9.18.sh"
...
if [[ -n "$PROBLEM" ]]; then
  ...
  if [[ "$TOKEN_RC" -ne 0 ]]; then
    ...
    {
      printf '%s\n' "$KEEP"
      printf '%s\n' 'SHELL=/bin/bash' 'PATH=/usr/local/bin:/usr/bin:/bin' '' '# BEGIN PROMOTE TO KING v2.9.18'
