#!/usr/bin/env bash
# Promote to King v2.9.22.9 -> v2.9.22.10 task telemetry restoration hotfix.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.9_to_v2.9.22.10_INCREMENTAL.zip"; ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.9_to_v2.9.22.10.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}; BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.9-to-v2.9.22.10.tar.gz"; RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.9-to-v2.9.22.10.tar.gz"
STAGE="$SITE_ROOT/.p2k-v292210-stage-$STAMP"; WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v292210-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.10" ]]; then echo "Promote to King is already VERSION 2.9.22.10; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.9" ]]&&PROBLEM="Expected production VERSION 2.9.22.9, found '$CURRENT'."
...
echo "===== P2K v2.9.22.9 -> v2.9.22.10 TASK TELEMETRY HOTFIX ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.9"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.10"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.9-to-v2.9.22.10" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.9 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
APPLY_OK=1
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.10" ]]||POST_OK=0
grep -q 'version: "2.9.22.10"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.10"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
grep -Fq 'const RUNTIME_VERSION = "2.9.22.10";' "$SITE_ROOT/assets/js/shared/api-client.js"||POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then (cd "$SITE_ROOT" && "$SHA_BIN" -c "$HASH_LIST" >/dev/null) || POST_OK=0; fi
if [[ "$POST_OK" -eq 1 ]]; then grep -q 'CORE_SCHEMA_VERSION = 16;' "$SITE_ROOT/server/team-points/src/Repository.php" || POST_OK=0; grep -q 'ANALYTICS_SCHEMA_VERSION = 7;' "$SITE_ROOT/server/team-points/src/Repository.php" || POST_OK=0; fi
if [[ "$POST_OK" -eq 1 ]]; then
  ! grep -Fq 'state.taskDetails.clear()' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  grep -Fq 'taskDetailLoads: new Map()' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  grep -Fq 'loadSelectedTaskDetail({ quiet: true })' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  grep -Fq "detail_mode'=>'lane-local-telemetry-v292210'" "$SITE_ROOT/server/control/public/api.php" || POST_OK=0
  grep -Fq "'club_index_verified_age_seconds'" "$SITE_ROOT/server/control/public/api.php" || POST_OK=0
  grep -Fq "'player_matches_server_verified_percent'" "$SITE_ROOT/server/control/public/api.php" || POST_OK=0
  grep -Fq 'No runnable job — ' "$SITE_ROOT/server/team-points/src/Worker.php" || POST_OK=0
  grep -Fq "'worker_idle_reason'" "$SITE_ROOT/server/team-points/public/cron.php" || POST_OK=0
  grep -Fq "'worker_idle_reason'" "$SITE_ROOT/server/control/public/api.php" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then
  bad_marker=""
  while IFS= read -r page; do
    while IFS= read -r marker; do [[ -z "$marker" || "$marker" == '?v=2.9.22.10' || "$marker" == '&v=2.9.22.10' ]] || { bad_marker="$page:$marker"; break 2; }; done < <(grep -Eo '[?&]v=[^"'"'"'<>[:space:]&]+' "$page" 2>/dev/null || true)
...
if [[ "$POST_OK" -eq 1 ]]; then node_bin=$(command -v node||true); [[ -n "$node_bin" ]]&&for f in assets/js/site-config.js assets/js/pages/task-control.js assets/js/shared/api-client.js; do "$node_bin" --check "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
if [[ "$POST_OK" -eq 1 ]]; then php_bin=$(command -v php||true); [[ -n "$php_bin" ]]&&for f in server/control/public/api.php server/team-points/public/cron.php server/team-points/src/Worker.php; do "$php_bin" -l "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
...
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.10 task telemetry hotfix installed."; echo "Core 16 / Analytics 7 and existing v2.9.22 CRON remain unchanged; no schema migration is required."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
rollback; echo "ERROR: v2.9.22.10 validation failed; release files were rolled back." >&2; exit 10
