#!/usr/bin/env bash
# Promote to King v2.9.11 -> v2.9.12 verified incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.11_to_v2.9.12_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.11_to_v2.9.12.json"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$SITE_ROOT/_backup/${STAMP}_v2.9.11_to_v2.9.12"
STAGE="$SITE_ROOT/.p2k-v299-stage-$STAMP"
...
if [[ "$CURRENT" == "2.9.12" ]]; then
  echo "Promote to King is already VERSION 2.9.12; no files were changed."
  exit 0
fi
[[ "$CURRENT" != "2.9.11" ]] && PROBLEM="Expected verified production VERSION 2.9.11, found '$CURRENT'."
...
echo "===== P2K v2.9.11 -> v2.9.12 VERIFIED INCREMENTAL UPDATE ====="
echo "Root:    $SITE_ROOT"
...
"$PHP_BIN" -r '
  ...
  if(!is_array($m)||($m["from"]??"")!=="2.9.11"||($m["to"]??"")!=="2.9.12"||!is_array($m["files"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
...
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.12" ]] || POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then
  grep -q 'version: "2.9.12"' "$SITE_ROOT/assets/js/site-config.js" || POST_OK=0
  grep -q '"version": "2.9.12"' "$SITE_ROOT/site-manifest.json" || POST_OK=0
  grep -q 'CORE_SCHEMA_VERSION = 12' "$SITE_ROOT/server/team-points/src/Repository.php" || POST_OK=0
fi

if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== VERIFY EXISTING CORE 12 / ANALYTICS 6 SCHEMA ====="
  "$PHP_BIN" -r '
    ...
    if(!$repo->schemaInstalled()||$repo->schemaVersion()<12||$repo->analyticsSchemaVersion()<6){fwrite(STDERR,"Schema validation failed; v2.9.12 does not mutate schema.\n");exit(2);}
...
if [[ "$POST_OK" -eq 1 ]]; then
  for f in "$SITE_ROOT/cron-dispatch-v2.9.12.sh" "$SITE_ROOT/reset-install-cron-v2.9.12.sh" "$SITE_ROOT/install-oauth-v2.9.12.sh"; do
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== INSTALL / VERIFY v2.9.12 CRON ====="
  P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" P2K_PRESERVE_SHARED_CONFIG=1 "$SITE_ROOT/reset-install-cron-v2.9.12.sh" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then
  for rel in server/team-points/config/config.local.php server/team-points/config/oauth.local.php data/server-config.json; do
    before="$BACKUP/$rel"; after="$SITE_ROOT/$rel"
...
if [[ "$POST_OK" -eq 1 ]]; then
  ...
  COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.12.sh' || true)
  [[ "$COUNT" -eq 4 ]] || { echo "CRON verification found $COUNT v2.9.12 entries, expected 4." >&2; POST_OK=0; }
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "SUCCESS: Promote to King v2.9.12 installed from verified v2.9.11."
  echo "Backup: $BACKUP"
...
rollback_update
echo "ERROR: v2.9.12 validation failed; changed release files and CRON state were rolled back." >&2
exit 10
