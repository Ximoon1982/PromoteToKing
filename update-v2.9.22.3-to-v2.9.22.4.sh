#!/usr/bin/env bash
# Promote to King v2.9.22.3 -> v2.9.22.4 verified Fresh Club Reconstruction updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.3_to_v2.9.22.4_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.3_to_v2.9.22.4.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.3-to-v2.9.22.4.tar.gz"
RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.3-to-v2.9.22.4.tar.gz"
STAGE="$SITE_ROOT/.p2k-v29224-stage-$STAMP"
WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v29224-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.4" ]]; then echo "Promote to King is already VERSION 2.9.22.4; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.3" ]]&&PROBLEM="Expected production VERSION 2.9.22.3, found '$CURRENT'."
...
echo "===== P2K v2.9.22.3 -> v2.9.22.4 FRESH CLUB RECONSTRUCTION UPDATE ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.3"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.4"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
grep -Eq '"removed"[[:space:]]*:[[:space:]]*\[[[:space:]]*\]' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: this minor update must not remove files." >&2; exit 5; }
...
while IFS= read -r rel; do
  [[ -n "$rel" ]] || continue
  case "$rel" in
    /*|*..*|data/*|logs/*|server/team-points/config/config.local.php|server/team-points/config/oauth.local.php|data/server-config.json)
      echo "ERROR: unsafe/protected payload path: $rel" >&2; exit 5 ;;
  esac
done < "$APPLY_LIST"
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.3-to-v2.9.22.4" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.3 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.4" ]]||POST_OK=0
grep -q 'version: "2.9.22.4"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.4"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  grep -Fq 'const boards=Number(payload?.boards||0);' "$SITE_ROOT/assets/js/pages/fresh-points-reconstruction.js" || POST_OK=0
  grep -Fq 'reconstruction-recalculate-club' "$SITE_ROOT/server/control/public/api.php" || POST_OK=0
  grep -Fq 'reconstructionRetryClubIssues' "$SITE_ROOT/TaskControl.html" || POST_OK=0
  grep -Fq "'wins'=>" "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then node_bin=$(command -v node||true); [[ -n "$node_bin" ]]&&for f in assets/js/shared/api-client.js assets/js/site-config.js assets/js/pages/fresh-points-reconstruction.js assets/js/pages/task-control.js; do "$node_bin" --check "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
...
if [[ "$POST_OK" -eq 1 && -n "$CURL_BIN" ]]; then http=$($CURL_BIN --silent --location --connect-timeout 8 --max-time 20 --output /dev/null --write-out '%{http_code}' "$BASE_URL/" 2>/dev/null||printf 000); [[ "$http" =~ ^2[0-9][0-9]$ ]]||echo "WARNING: external site probe returned HTTP $http; file-integrity validation remains authoritative for this artwork update." >&2; fi
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.4 Fresh Club Reconstruction update installed."; echo "Core 15 / Analytics 7 / existing v2.9.22 CRON contract retained unchanged."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
rollback; echo "ERROR: v2.9.22.4 validation failed; release files were rolled back." >&2; exit 10
