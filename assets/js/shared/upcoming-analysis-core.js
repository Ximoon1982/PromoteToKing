/* Shared Upcoming Matches and embedded single-match analysis core. */
(async () => {
  "use strict";
  if (window.P2K_ADMIN_ACCESS_READY && !(await window.P2K_ADMIN_ACCESS_READY)) return;

  const MODE = Object.freeze({
    embeddedSingleMatch: false,
    includeCopy: true,
    includeStartInRecruitText: false,
    ...(window.P2K_UPCOMING_MODE || {})
  });
  ...
  const upcomingMatchesTab = document.getElementById("p2kUpcomingMatchesTab");
  const upcomingPlayersTab = document.getElementById("p2kUpcomingPlayersTab");
  const upcomingMatchesPanel = document.getElementById("p2kUpcomingMatchesPanel");
  const upcomingPlayersPanel = document.getElementById("p2kUpcomingPlayersPanel");
  const playerSummary = document.getElementById("p2kPlayerSummary");
  const playerResults = document.getElementById("p2kPlayerResults");
  const playerSearchInput = document.getElementById("p2kPlayerSearch");
  const clearPlayerSearchButton = document.getElementById("p2kClearPlayerSearch");
  const playerSortSelect = document.getElementById("p2kPlayerSort");
  ...
  const state = {
    ...
    failedReferences: [],
    failedDetails: [],
    playerSortMode: "registrations-desc",
    visibleCount: PAGE_SIZE,
    expandedDetails: new Set(),
    recruitmentRecapExpanded: false,
    upcomingSection: "matches"
  };
  ...
  function showCancelButton() {
    ...
    cancelButton.title = "Cancel processing while retaining completed results";
  }

  function showRetryFailedButton() {
    processingActions.style.display = "flex";
    cancelButton.disabled = false;
    cancelButton.dataset.p2kAction = "retry-failed";
    cancelButton.textContent = "Retry failed";
    cancelButton.title = "Retry only match detail requests that failed";
  }

  function hideCancelButton() {
    processingActions.style.display = "none";
    cancelButton.disabled = false;
    delete cancelButton.dataset.p2kAction;
    cancelButton.textContent = "Cancel";
  ...
  function recruitmentTargetReached(comparison, targetBoardCount, isLeagueMatch) {
    ...
    /* Friendly recruitment stops once P2K is at least as likely to win as the
       opponent. The caller separately enforces the minimum-player requirement
       and a P2K lineup count at least equal to the opponent's usable lineup. */
    return comparison.p2kWinProbability >= comparison.opponentWinProbability;
  }
  ...
  function buildRecruitmentRecommendation(match, p2kTeam, opponentTeam, bounds, maxPlayers, minPlayers, isLeagueMatch) {
    ...
    const targetLabel = isLeagueMatch
      ...
      : "at least the match minimum, at least as many eligible players as the opponent's usable lineup, and a projected P2K win probability at least as high as the opponent's";
  ...
  function analyzeMatch(match) {
    ...
    if (!hasEnoughEligiblePlayers) {
      detailedAnalysis.p2kWinProbability = 0;
      detailedAnalysis.opponentWinProbability = 1;
      detailedAnalysis.drawProbability = 0;
      detailedAnalysis.minimumEligibilityForfeit = true;
    }
  ...
  function matchRuleCategory(match) {
    const rules = String(match?.raw?.settings?.rules || "").toLowerCase();
    return rules.includes("960") ? "chess960" : "classical";
  }

  function aggregateRegisteredPlayers() {
    const rows = new Map();

    state.matches.forEach(match => {
      const category = matchRuleCategory(match);
      players(match.p2kTeam).forEach(player => {
        ...
        if (!row) {
          row = {
            ...
            classicalRatings: [],
            chess960Ratings: [],
            classicalTimeoutRates: [],
            chess960TimeoutRates: [],
            matches: []
          };
          rows.set(key, row);
        }
        row.registrations += 1;
        const rating = Number(player?.rating);
        const timeout = playerTimeoutRate(player);
        const ratingList = category === "chess960" ? row.chess960Ratings : row.classicalRatings;
        const timeoutList = category === "chess960" ? row.chess960TimeoutRates : row.classicalTimeoutRates;
        if (Number.isFinite(rating) && rating > 0) ratingList.push(rating);
        if (timeout !== null) timeoutList.push(timeout);
        row.matches.push({
          name: match.name,
          url: match.url,
          opponent: String(match.opponentTeam?.name || "Opponent"),
          category,
    ...
    return [...rows.values()].map(row => ({
      ...row,
      classicalRating: medianNumber(row.classicalRatings),
      chess960Rating: medianNumber(row.chess960Ratings),
      classicalTimeoutRate: medianNumber(row.classicalTimeoutRates),
      chess960TimeoutRate: medianNumber(row.chess960TimeoutRates)
    }));
  }
  ...
  function playerSortValue(row, field) {
    if (field === "classical") return row.classicalRating;
    if (field === "chess960") return row.chess960Rating;
    if (field === "classicalTimeout") return row.classicalTimeoutRate;
    if (field === "chess960Timeout") return row.chess960TimeoutRate;
    if (field === "registrations") return row.registrations;
    return row.username;
  }

  function setPlayerSort(mode, render = true) {
    state.playerSortMode = String(mode || "registrations-desc");
    if (playerSortSelect) playerSortSelect.value = state.playerSortMode;
    if (render) renderPlayers();
  }

  function togglePlayerSort(field) {
    const current = state.playerSortMode || "registrations-desc";
    const [currentField, currentDirection] = current.split("-");
    const defaultDirection = field === "name" || field.includes("Timeout") ? "asc" : "desc";
    const direction = currentField === field
      ? (currentDirection === "asc" ? "desc" : "asc")
      : defaultDirection;
    setPlayerSort(`${field}-${direction}`);
  }

  function sortedRegisteredPlayers(rows) {
    const mode = state.playerSortMode || playerSortSelect?.value || "registrations-desc";
    const splitAt = mode.lastIndexOf("-");
    const field = splitAt > 0 ? mode.slice(0, splitAt) : "registrations";
    const direction = splitAt > 0 ? mode.slice(splitAt + 1) : "desc";
    return [...rows].sort((left, right) => {
      let result = 0;
      if (field === "name") {
        result = left.username.localeCompare(right.username, undefined, { sensitivity: "base" });
        if (direction === "desc") result *= -1;
      } else if (field === "registrations") {
        result = direction === "asc" ? left.registrations - right.registrations : right.registrations - left.registrations;
      } else {
        result = compareNullableNumbers(playerSortValue(left, field), playerSortValue(right, field), direction);
      }
      return result || left.username.localeCompare(right.username, undefined, { sensitivity: "base" });
    });
  }

  function playerSortHeader(field, label) {
    const [activeField, direction] = (state.playerSortMode || "").split(/-(?=[^-]+$)/);
    const active = activeField === field;
    return `<button class="p2k-player-sort-button${active ? " is-active" : ""}" data-player-sort-field="${escapeHTML(field)}" type="button">${escapeHTML(label)}<span aria-hidden="true">${active ? (direction === "asc" ? "▲" : "▼") : "↕"}</span></button>`;
  }

  function openPlayerMatches(player, trigger) {
    const rows = [...player.matches].sort((a, b) => Number(a.startTimestamp || 0) - Number(b.startTimestamp || 0));
    openSharedChartModal({
      titleHTML: `${escapeHTML(player.username)} — registered matches (${rows.length})`,
      trigger,
      bodyHTML: `<p class="p2k-player-profile-action"><a href="https://www.chess.com/member/${encodeURIComponent(player.username)}" target="_blank" rel="noopener noreferrer">Open ${escapeHTML(player.username)}’s Chess.com profile</a></p><div class="p2k-player-match-list">${rows.map(match => `
        <a class="p2k-player-match-row" href="${escapeHTML(match.url)}" target="_blank" rel="noopener noreferrer">
          <span class="p2k-player-match-main"><strong>${escapeHTML(match.name)}</strong><small>vs ${escapeHTML(match.opponent)} · ${match.category === "chess960" ? "Chess960" : "Classical"}</small></span>
          <span class="p2k-player-match-values"><span>Rating ${Number.isFinite(match.rating) ? Math.round(match.rating) : "—"}</span><span>Timeout ${Number.isFinite(match.timeout) ? `${Math.round(match.timeout * 10) / 10}%` : "—"}</span></span>
        </a>`).join("")}</div>`
    });
  }

  function renderPlayers() {
    if (!playerResults || !playerSummary) return;
    if (!state.processed) {
      playerSummary.innerHTML = "";
      playerResults.innerHTML = '<div class="p2k-empty">Analyze upcoming matches to build the registered-player list.</div>';
      return;
    }

    const allPlayers = aggregateRegisteredPlayers();
    const totalRegistrations = allPlayers.reduce((sum, player) => sum + player.registrations, 0);
    const classicalCount = allPlayers.filter(player => Number.isFinite(player.classicalRating)).length;
    const chess960Count = allPlayers.filter(player => Number.isFinite(player.chess960Rating)).length;
    playerSummary.innerHTML = `
      <div class="p2k-player-stat"><span>Unique players</span><strong>${allPlayers.length}</strong></div>
      <div class="p2k-player-stat"><span>Total registrations</span><strong>${totalRegistrations}</strong></div>
      <div class="p2k-player-stat"><span>Classical ratings</span><strong>${classicalCount}</strong></div>
      <div class="p2k-player-stat"><span>Chess960 ratings</span><strong>${chess960Count}</strong></div>
    `;

    const query = normalizedSearchText(playerSearchInput?.value);
    const filtered = sortedRegisteredPlayers(allPlayers.filter(player => !query || normalizedSearchText(player.username).includes(query)));

    if (!filtered.length) {
      playerResults.innerHTML = `<div class="p2k-empty">${allPlayers.length ? "No registered player matches this search." : "No Promote to King registration was found in the analyzed matches."}</div>`;
      return;
    }

    const ratingCell = value => Number.isFinite(value) ? String(Math.round(value)) : '<span class="p2k-player-missing">—</span>';
    const timeoutCell = value => Number.isFinite(value) ? `${Math.round(value * 10) / 10}%` : '<span class="p2k-player-missing">—</span>';

    playerResults.innerHTML = `
      <p class="p2k-player-table-caption">Showing ${filtered.length} of ${allPlayers.length} registered player${allPlayers.length === 1 ? "" : "s"}. Values are medians of the relevant classical or Chess960 match-sheet observations.</p>
      <div class="p2k-player-table-wrap">
        <table class="p2k-player-table" aria-label="Registered Promote to King players">
          <thead><tr>
            <th scope="col">${playerSortHeader("name", "Player")}</th>
            <th scope="col">${playerSortHeader("classical", "Classical")}</th>
            <th scope="col">${playerSortHeader("chess960", "Chess960")}</th>
            <th scope="col">${playerSortHeader("classicalTimeout", "Classical timeout")}</th>
            <th scope="col">${playerSortHeader("chess960Timeout", "Chess960 timeout")}</th>
            <th scope="col">${playerSortHeader("registrations", "Registrations")}</th>
          </tr></thead>
          <tbody>${filtered.map(player => `<tr class="p2k-player-row" data-player-key="${escapeHTML(player.key)}" tabindex="0" role="button" aria-label="Show matches for ${escapeHTML(player.username)}">
            <td><span class="p2k-player-name">${escapeHTML(player.username)}</span></td>
            <td>${ratingCell(player.classicalRating)}</td>
            <td>${ratingCell(player.chess960Rating)}</td>
            <td>${timeoutCell(player.classicalTimeoutRate)}</td>
            <td>${timeoutCell(player.chess960TimeoutRate)}</td>
            <td>${player.registrations}</td>
          </tr>`).join("")}</tbody>
        </table>
      </div>`;

    playerResults.querySelectorAll("[data-player-sort-field]").forEach(button => button.addEventListener("click", () => togglePlayerSort(button.dataset.playerSortField)));
    const rowsByKey = new Map(allPlayers.map(player => [player.key, player]));
    playerResults.querySelectorAll(".p2k-player-row").forEach(row => {
      const activate = () => { const player = rowsByKey.get(row.dataset.playerKey); if (player) openPlayerMatches(player, row); };
  ...
  function setUpcomingSection(section, focus = true) {
    if (!upcomingMatchesTab || !upcomingPlayersTab || !upcomingMatchesPanel || !upcomingPlayersPanel) return;
    const playersSelected = section === "players";
    state.upcomingSection = playersSelected ? "players" : "matches";
    upcomingMatchesTab.setAttribute("aria-selected", playersSelected ? "false" : "true");
    upcomingPlayersTab.setAttribute("aria-selected", playersSelected ? "true" : "false");
    upcomingMatchesTab.tabIndex = playersSelected ? -1 : 0;
    upcomingPlayersTab.tabIndex = playersSelected ? 0 : -1;
    upcomingMatchesPanel.hidden = playersSelected;
    upcomingPlayersPanel.hidden = !playersSelected;
    if (playersSelected) renderPlayers();
    if (focus) (playersSelected ? upcomingPlayersTab : upcomingMatchesTab).focus({ preventScroll: true });
  }
  ...
  function visibleModalRegion() {
    const ownHeight = Math.max(document.documentElement.scrollHeight, document.body?.scrollHeight || 0, window.innerHeight || 0);
    ...
    try {
      if (window.parent !== window && window.frameElement) {
        ...
        const parentBottom = parentTop + (parentViewport?.height || window.parent.innerHeight || frameRect.height);
    ...
    if (!Number.isFinite(bottom) || bottom <= top) bottom = Math.min(ownHeight, top + (window.innerHeight || 600));
    return { top: Math.max(0, top), bottom: Math.max(top + 1, bottom), height: Math.max(1, bottom - top), ownHeight };
  }

  function positionModalInVisibleViewport(modal) {
    if (!modal || modal.hidden) return;
    const region = visibleModalRegion();
    const margin = Math.min(18, Math.max(8, Math.floor(region.height * .04)));
    ...
    const dialog = modal.querySelector(".p2k-chart-modal-dialog");
    if (dialog) dialog.style.maxHeight = `${Math.max(180, region.height - margin * 2)}px`;
  }

  function positionOpenModals() {
    [chartModal, copyChoiceModal, copyModal].forEach(positionModalInVisibleViewport);
  }

  function installModalViewportListeners() {
    const schedule = () => window.requestAnimationFrame(positionOpenModals);
  ...
  function openSharedChartModal({ titleHTML = "", bodyHTML = "", trigger = null, afterRender = null } = {}) {
    if (!chartModal || !chartModalTitle || !chartModalBody || !chartModalClose) return false;
    lastChartModalTrigger = trigger;
    chartModalTitle.innerHTML = String(titleHTML || "");
    chartModalBody.innerHTML = String(bodyHTML || "");
    chartModal.hidden = false;
    chartModal.setAttribute("aria-hidden", "false");
    positionModalInVisibleViewport(chartModal);
    if (typeof afterRender === "function") afterRender(chartModalBody);
  ...
  function openChartModal(matchKey, chartType = "distribution", triggerButton = null) {
    ...
    const cumulative = chartType === "cumulative";
    openSharedChartModal({
      titleHTML: cumulative
        ? `${escapeHTML(match.name)} — cumulative players at or above rating${infoMarkHTML("Each point is the number of registered players rated at or above that 100-Elo threshold. Hover or tap a point for the exact total.", "Cumulative chart information")}`
        : `${escapeHTML(match.name)} — registered players by rating bracket${infoMarkHTML("Bars show registered players in each 100-Elo bracket. Solid bars are rating-eligible; translucent bars are outside the match rating window. Hover or tap a bar for the exact count.", "Rating bracket chart information")}`,
      bodyHTML: cumulative
        ? renderCumulativeRatingHTML(match.detailedAnalysis.cumulativeRatingData, p2kName, opponentName, { large: true })
        : renderRatingHistogramHTML(match.detailedAnalysis.ratingBracketHistogram, match.detailedAnalysis.ratingStatistics, p2kName, opponentName, { large: true }),
  ...
  window.P2K_UPCOMING_CHART_MODAL = Object.freeze({
    open: openSharedChartModal,
  ...
  function recruitmentPanelHTML(match) {
    ...
    const goalInformation = match.isLeagueMatch
      ...
      : "Friendly matches recruit until P2K meets the match minimum, has at least as many eligible players as the opponent's usable lineup, and is at least as likely to win as the opponent. The model recommends the lowest tested player count reaching all three conditions.";
    ...
    recruitmentRecap.innerHTML = `
      <div class="p2k-recruitment-recap-header">
        <div class="p2k-recruitment-recap-title">
          ...
          ${infoMarkHTML("Only matches starting during the next 10 days are included. League matches are listed first. Friendly matches remain listed whenever P2K is below the match minimum, has fewer eligible players than the opponent's usable lineup, or is less likely to win. Rating ranges are rounded down at the lower bound and up at the upper bound to the nearest hundred.", "Recruitment recap methodology")}
        </div>
  ...
  function detailedInfoHTML(match) {
    ...
    const historyChart = window.P2K_MATCH_HISTORY_UI?.placeholderHTML?.(
      match.raw?.["@id"] || match.url || match.cacheKey
    ) || "";
...
` : ""}
  ...
  function openCopyChoice(payload) {
    ...
    positionModalInVisibleViewport(copyChoiceModal);
  ...
  function showManualCopyModal(text, title = "Copy HTML source manually") {
    ...
    positionModalInVisibleViewport(copyModal);
    document.body.style.overflow = "hidden";
  ...
  function recruitmentRecapPlainText() {
    ...
    const lineForMatch = match => {
      ...
      const leaguePrefix = match.isLeagueMatch
        ...
        : "";
      return `${leaguePrefix}${count} ${playerWord} in range ${recruitmentPlainTextRange(recommendation)} needed vs ${opponent} @ ${match.url}${MODE.includeStartInRecruitText ? ` starts ${formatRecruitmentStartDate(match.startTimestamp)}` : ""}.`;
    };
  ...
  async function analyzeUpcomingMatches(options = {}) {
    ...
    try {
      ...
      if (total === 0) {
        ...
        renderResults();
        window.P2K_ANALYSIS_COORDINATOR?.complete?.("upcoming", { synchronized, failedCount: 0, processedAt: Date.now() });
        return;
      }

      // Feed the shared scheduler with the full detail reservoir. Public/fake OAuth
      // remains serial; real OAuth lets the server-side adaptive gateway determine
      // the safe concurrency instead of hard-coding one match at a time.
      const detailBatch = await window.P2K_API_CLIENT.processPriority(
        registrationMatches,
        async reference => {
          if (signal.aborted) throw new CancellationError();
          const matchData = await loadJSON(reference["@id"], signal);
          return analyzeMatch(matchData);
        },
        {
          signal,
          getKey: reference => String(reference?.["@id"] || ""),
          onProgress: progressState => {
            const done = progressState.settled;
            setStatus(`Processing match ${Math.min(done + 1, total)} of ${total}`, "working", total ? (done / total) * 100 : 100);
            progressBar.style.width = `${total ? (done / total) * 100 : 100}%`;
          }
        }
      );
      state.matches = detailBatch.succeeded
        .sort((a,b)=>a.index-b.index)
        .map(entry => entry.value)
        .filter(Boolean);
      detailBatch.failures.forEach(failure => {
        state.failedReferences.push(failure.item);
        state.failedDetails.push({ reference: failure.item, error: failure.error, phase: "Match detail" });
        console.warn("P2K Upcoming Matches Analyzer: skipped match", failure.item, failure.error);
      });
      state.failedCount = state.failedReferences.length;
      if (detailBatch.cancelled || signal.aborted) throw new CancellationError();
      ...
      try {
        ...
        state.failedReferences = state.failedReferences.filter(reference =>
          currentRegisteredIDs.has(String(reference?.["@id"] || ""))
        );
        state.failedDetails = state.failedDetails.filter(detail =>
          currentRegisteredIDs.has(String(detail?.reference?.["@id"] || detail?.url || ""))
        );
        state.failedCount = state.failedReferences.length;
        ...
        state.failedDetails.push({
          error: refreshError,
          apiUrl: refreshError?.url || MATCH_LIST_URL,
          phase: "Final registration refresh",
          matchName: "Current registration match list"
        });
      ...
      const removalNote = matchesRemovedAfterRefresh > 0
        ...
        : "";
      const requestFailureCount = state.failedDetails.length;
      const nonDetailFailureCount = Math.max(0, requestFailureCount - state.failedCount);
      setStatus(
        `Analysis complete: ${state.matches.length} current registration match${state.matches.length === 1 ? "" : "es"} processed.${state.failedCount ? ` ${state.failedCount} match detail request${state.failedCount === 1 ? "" : "s"} failed.` : ""}${nonDetailFailureCount ? ` ${nonDetailFailureCount} additional API request${nonDetailFailureCount === 1 ? "" : "s"} failed.` : ""}${removalNote}`,
        requestFailureCount ? "error" : "success",
        100
      );
      window.CLUB_ANALYSIS_FAILURE_UI?.attach?.(statusText, state.failedDetails, { title: "Upcoming Matches analysis failures" });
      renderResults();
      window.P2K_ANALYSIS_COORDINATOR?.complete?.("upcoming", {
        synchronized,
      ...
      if (error instanceof CancellationError || signal.aborted) {
        ...
        window.CLUB_ANALYSIS_FAILURE_UI?.attach?.(statusText, [{ error, url: error?.url || MATCH_LIST_URL, phase: "Match list or analysis", matchName: "Upcoming matches analysis" }], { title: "Upcoming Matches analysis failure" });
      }
    } finally {
      if (activeAnalysis === controller) activeAnalysis = null;
      state.processing = false;
      if (state.failedReferences.length) showRetryFailedButton();
      else hideCancelButton();
      analyzeButton.disabled = false;
      analyzeButton.textContent = MODE.embeddedSingleMatch ? (state.processed ? "Refresh upcoming matches" : "Analyze upcoming matches") : "Analyze";
    }
  }

  async function retryFailedMatches() {
    if (state.processing || !state.failedReferences.length) return;

    const controller = new AbortController();
    activeAnalysis = controller;
    const signal = controller.signal;
    const retryReferences = [...state.failedReferences];
    const remainingFailures = [];
    const remainingFailureDetails = [];
    ...
    try {
      const retryBatch = await window.P2K_API_CLIENT.processPriority(
        retryReferences,
        async reference => {
          const matchData = await loadJSON(reference["@id"], signal, { networkOnly: true });
          return analyzeMatch(matchData);
        },
        {
          signal,
          getKey: (reference, index) => String(reference?.["@id"] || index),
          onProgress: progress => {
            completedRetries = progress.settled;
            setStatus(
              `Retrying failed matches ${progress.settled} of ${progress.total}`,
              "working",
              progress.total ? (progress.settled / progress.total) * 100 : 100
            );
          }
        }
      );
      if (retryBatch.cancelled || signal.aborted) throw new CancellationError();
      for (const entry of retryBatch.succeeded) {
        const analyzedMatch = entry.value;
        if (!analyzedMatch) continue;
        const key = String(analyzedMatch.raw?.["@id"] || analyzedMatch.cacheKey || "");
        const existingIndex = state.matches.findIndex(match =>
          String(match.raw?.["@id"] || match.cacheKey || "") === key
        );
        if (existingIndex >= 0) state.matches[existingIndex] = analyzedMatch;
        else state.matches.push(analyzedMatch);
      }
      for (const entry of retryBatch.failures) {
        remainingFailures.push(entry.item);
        remainingFailureDetails.push({ reference: entry.item, error: entry.error, phase: "Retry match detail" });
        console.warn("P2K Upcoming Matches Analyzer: retry failed", entry.item, entry.error);
      }

      state.failedReferences = remainingFailures;
      state.failedDetails = remainingFailureDetails;
      state.failedCount = remainingFailures.length;
      state.processed = true;
      renderResults();
      setStatus(
        remainingFailures.length
          ? `Retry complete: ${remainingFailures.length} match detail request${remainingFailures.length === 1 ? "" : "s"} still failed.`
          : "All failed match detail requests were loaded successfully.",
        remainingFailures.length ? "error" : "success",
        100
      );
      window.CLUB_ANALYSIS_FAILURE_UI?.attach?.(statusText, state.failedDetails, { title: "Retry failures" });
    } catch (error) {
      if (error instanceof CancellationError || signal.aborted) {
        state.failedReferences = [
          ...remainingFailures,
          ...retryReferences.slice(completedRetries)
        ];
        state.failedCount = state.failedReferences.length;
        renderResults();
        setStatus("Retry cancelled. Previously loaded results were retained.", "working", null);
      } else {
        console.error(error);
        state.failedReferences = [...remainingFailures, ...retryReferences];
        state.failedCount = state.failedReferences.length;
        state.failedDetails = [
          ...remainingFailureDetails,
          { error, apiUrl: error?.url || "", phase: "Retry failed matches", matchName: "Retry operation" }
        ];
        renderResults();
        setStatus(`Unable to retry failed matches: ${error.message || error}`, "error", null);
        window.CLUB_ANALYSIS_FAILURE_UI?.attach?.(statusText, state.failedDetails, { title: "Retry failures" });
      }
    } finally {
      if (activeAnalysis === controller) activeAnalysis = null;
  ...
  window.P2K_MATCH_HISTORY_SUMMARIZER = rawMatch => {
    ...
    return {
      p2kName: analysis.p2kTeam?.name || "Promote to King",
      opponentName: analysis.opponentTeam?.name || "Opponent",
      p2kCount: analysis.p2kPlayerCount,
      opponentCount: analysis.opponentPlayerCount,
      minPlayers: analysis.minPlayers,
      p2kWinProbability: analysis.detailedAnalysis.p2kWinProbability
    };
  };
  ...
  function normalizeEmbeddedMatchReference(value) {
    const text = String(value || "").trim();
    if (!text) return null;
    const match = text.match(/(\d+)(?:\/?(?:[?#].*)?)?$/);
    return match ? match[1] : (/^\d+$/.test(text) ? text : null);
  }

  async function analyzeEmbeddedSingleMatch() {
    const requested = new URLSearchParams(window.location.search).get("match");
    const matchId = normalizeEmbeddedMatchReference(requested);
    if (!matchId) {
      state.processed = true;
      setStatus("No valid match ID was supplied.", "error", null);
      resultsBox.innerHTML = '<div class="p2k-empty">Unable to open detailed analysis because the match reference is missing or invalid.</div>';
      return;
    }
    const controller = new AbortController();
    activeAnalysis = controller;
    ...
    state.expandedDetails.clear();
    try {
      setStatus("Loading detailed match analysis…", "working", 12);
      await allowScreenUpdate();
      const matchData = await loadJSON(`https://api.chess.com/pub/match/${encodeURIComponent(matchId)}`, controller.signal);
      const analyzedMatch = analyzeMatch(matchData);
      if (!analyzedMatch) throw new Error("The two match teams could not be identified.");
      state.matches = [analyzedMatch];
      state.expandedDetails.add(analyzedMatch.cacheKey);
      state.processed = true;
      const includeLocked = root.querySelector('input[name="p2kLockFilter"][value="all"]');
      if (includeLocked) includeLocked.checked = true;
      const showAll = root.querySelector('input[name="p2kLockableFilter"][value="all"]');
      if (showAll) showAll.checked = true;
      setStatus("Detailed match analysis loaded.", "success", 100);
      renderResults();
      window.setTimeout(() => { if (statusBox) statusBox.style.display = "none"; }, 250);
    } catch (error) {
      console.error("P2K embedded detailed match analysis", error);
      state.processed = true;
      setStatus(`Unable to analyze this match: ${window.P2K_API_CLIENT?.userMessage?.(error) || error.message || error}`, "error", null);
      resultsBox.innerHTML = '<div class="p2k-empty">Detailed match analysis could not be loaded.</div>';
    } finally {
      if (activeAnalysis === controller) activeAnalysis = null;
      state.processing = false;
    }
  ...
  analyzeButton.addEventListener("click", () => { void analyzeUpcomingMatches(); });
  if (!MODE.embeddedSingleMatch) {
    window.P2K_ANALYSIS_COORDINATOR?.register?.("upcoming", {
      isBusy: () => state.processing,
      canRefresh: () => true,
      refresh: ({ synchronized = false } = {}) => analyzeUpcomingMatches({ synchronized })
    });
  }
  ...
  upcomingMatchesTab?.addEventListener("click", () => setUpcomingSection("matches", false));
  upcomingPlayersTab?.addEventListener("click", () => setUpcomingSection("players", false));
  [upcomingMatchesTab, upcomingPlayersTab].filter(Boolean).forEach(tab => {
    tab.addEventListener("keydown", event => {
      if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
      event.preventDefault();
      const target = event.key === 'ArrowRight' || event.key === 'End' ? 'players' : 'matches';
      setUpcomingSection(target, true);
    });
  });

  playerSearchInput?.addEventListener("input", () => {
    if (clearPlayerSearchButton) clearPlayerSearchButton.disabled = playerSearchInput.value.length === 0;
    renderPlayers();
  });
  clearPlayerSearchButton?.addEventListener("click", () => {
    if (!playerSearchInput?.value) return;
    playerSearchInput.value = "";
    clearPlayerSearchButton.disabled = true;
    renderPlayers();
    playerSearchInput.focus();
  });
  playerSortSelect?.addEventListener("change", () => setPlayerSort(playerSortSelect.value));
  if (clearPlayerSearchButton && playerSearchInput) {
    clearPlayerSearchButton.disabled = playerSearchInput.value.length === 0;
  }
  ...
  if (MODE.embeddedSingleMatch) void analyzeEmbeddedSingleMatch();
})();
