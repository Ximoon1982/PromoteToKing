(() => {
  ...
  function renderGreenControl(payload) {
    ...
    const gabm=$("greenGabMetrics");if(gabm)gabm.innerHTML=[greenMetric("Status",gab.status||s.gab_status||"not started"),greenMetric("Current lane",gab.phase||s.gab_phase||"—"),greenMetric("Overall",`${Number(gab.percent||0).toFixed(1)}%`,`${number(gab.completed_lanes||0)} / ${number(gab.total_lanes||0)} lanes complete`),greenMetric("External pending",number(gab.external?.pending||0)),greenMetric("External failed",number(gab.external?.failed||0)),greenMetric("Accelerator priority",String(gab.status)==="running"?"GAB first":"normal")].join("");setHTML("greenGabPhases",renderGreenBars(Array.isArray(gab.lanes)?gab.lanes:[]));
