(() => {
  ...
  function renderNativePie(hostId, rows, { valueKey = "value", labelKey = "label" } = {}) {
    ...
    if (usable.length === 1) { const row = usable[0], value = Number(row[valueKey]), color = row.color || palette[0], circle = nativeSVG("circle", { cx, cy, r: radius, fill: color }); bindChartTip(circle,host,()=>`<strong>${escapeHTML(String(row[labelKey]))}</strong><br>${number(value)} · 100.0%`); svg.appendChild(circle); const swatch = nativeSVG("rect", { x: 350, y: 39, width: 13, height: 13, rx: 2, fill: color }), label = nativeSVG("text", { x: 374, y: 50, class: "p2k-chart-axis" }); label.textContent = `${row[labelKey]} · ${number(value)} · 100.0%`; svg.append(swatch, label); return; }
