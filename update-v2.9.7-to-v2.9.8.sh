#!/usr/bin/env bash
# Promote to King v2.9.7 -> v2.9.8 verified incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.7_to_v2.9.8_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.7_to_v2.9.8.json"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$SITE_ROOT/_backup/${STAMP}_v2.9.7_to_v2.9.8"
STAGE="$SITE_ROOT/.p2k-v298-stage-$STAMP"
...
if [[ "$CURRENT" == "2.9.8" ]]; then
  echo "Promote to King is already VERSION 2.9.8; no files were changed."
  exit 0
fi
[[ "$CURRENT" != "2.9.7" ]] && PROBLEM="Expected verified production VERSION 2.9.7, found '$CURRENT'."
...
echo "===== P2K v2.9.7 -> v2.9.8 VERIFIED INCREMENTAL UPDATE ====="
echo "Root:    $SITE_ROOT"
...
"$PHP_BIN" -r '
  ...
  if(!is_array($m)||($m["from"]??"")!=="2.9.7"||($m["to"]??"")!=="2.9.8"||!is_array($m["files"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
...
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.8" ]] || POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then
  grep -q 'version: "2.9.8"' "$SITE_ROOT/assets/js/site-config.js" || POST_OK=0
  grep -q '"version": "2.9.8"' "$SITE_ROOT/site-manifest.json" || POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  for f in "$SITE_ROOT/cron-dispatch-v2.9.8.sh" "$SITE_ROOT/reset-install-cron-v2.9.8.sh" "$SITE_ROOT/install-oauth-v2.9.8.sh"; do
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== INSTALL / VERIFY v2.9.8 CRON ====="
  P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" "$SITE_ROOT/reset-install-cron-v2.9.8.sh" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then
  INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
  COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.8.sh' || true)
  [[ "$COUNT" -eq 4 ]] || { echo "CRON verification found $COUNT v2.9.8 entries, expected 4." >&2; POST_OK=0; }
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "SUCCESS: Promote to King v2.9.8 installed from verified v2.9.7."
  echo "Backup: $BACKUP"
...
rollback_update
echo "ERROR: v2.9.8 validation failed; changed release files and CRON state were rolled back." >&2
exit 10
