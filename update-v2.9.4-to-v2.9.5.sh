#!/usr/bin/env bash
# Promote to King v2.9.4 -> v2.9.5 argumentless incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.4_to_v2.9.5_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.4_to_v2.9.5.json"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$SITE_ROOT/_backup/${STAMP}_v2.9.4_to_v2.9.5"
STAGE="$SITE_ROOT/.p2k-v295-stage-$STAMP"
...
[[ "$CURRENT" != "2.9.4" && "$CURRENT" != "2.9.5" ]] && PROBLEM="Expected production VERSION 2.9.4 (or already-updated 2.9.5), found '$CURRENT'."
...
if [[ -n "$PROBLEM" ]]; then
  ...
  echo "===== P2K v2.9.4 -> v2.9.5 INCREMENTAL UPDATE ====="
  echo "Root:     $SITE_ROOT"
  ...
  if [[ "$EXTRACT_OK" -ne 1 || ! -f "$STAGE/$MANIFEST_NAME" ]]; then
    ...
    "$PHP_BIN" -r '
      ...
      if(!is_array($m)||($m["from"]??"")!=="2.9.4"||($m["to"]??"")!=="2.9.5"||!is_array($m["files"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
    ...
    if [[ "$VERIFY_RC" -ne 0 ]]; then
      ...
      # OAuth POC registration values are deployment-specific. If this site
      # already had a configured POC file, keep it rather than replacing it
      # with the packaged placeholder/example values.
      if [[ "$APPLY_OK" -eq 1 && -f "$BACKUP/config/oauth-test.php" ]]; then
        mkdir -p "$SITE_ROOT/config"
        cp -p "$BACKUP/config/oauth-test.php" "$SITE_ROOT/config/oauth-test.php" || APPLY_OK=0
      fi
      ...
      POST_OK=$APPLY_OK
      if [[ "$POST_OK" -eq 1 && "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" != "2.9.5" ]]; then echo "ERROR: VERSION validation failed." >&2; POST_OK=0; fi
      if [[ "$POST_OK" -eq 1 ]]; then
        grep -q 'country_code' "$SITE_ROOT/server/team-points/sql/core-migration-v2.9.5.sql" || POST_OK=0
        grep -q 'p2k_rating' "$SITE_ROOT/server/team-points/sql/core-migration-v2.9.5.sql" || POST_OK=0
      fi
      if [[ "$POST_OK" -eq 1 ]]; then bash -n "$SITE_ROOT/cron-dispatch-v2.9.5.sh" && bash -n "$SITE_ROOT/reset-install-cron-v2.9.5.sh" || POST_OK=0; fi
      if [[ "$POST_OK" -eq 1 ]]; then
        for f in \
      ...
      if [[ "$POST_OK" -eq 1 ]]; then
        echo "===== INSTALL / VERIFY v2.9.5 CRON ====="
        P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" "$SITE_ROOT/reset-install-cron-v2.9.5.sh"
        INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
        COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.5.sh' || true)
        [[ "$COUNT" -eq 4 ]] || { echo "ERROR: v2.9.5 CRON verification found $COUNT entries." >&2; POST_OK=0; }
      fi

      if [[ "$POST_OK" -eq 1 ]]; then
        echo "SUCCESS: Promote to King v2.9.5 installed."
        echo "Backup: $BACKUP"
      else
        rollback_update
        echo "ERROR: v2.9.5 update failed validation and was rolled back where feasible." >&2
      fi
    fi
  fi
  rm -rf "$STAGE" 2>/dev/null || true
