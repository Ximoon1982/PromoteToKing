#!/usr/bin/env bash
# Promote to King v2.9.19 - argumentless IONOS CRON installer.
...
DISPATCHER="$SITE_ROOT/cron-dispatch-v2.9.19.sh"
WEEKLY_BACKUP="$SITE_ROOT/weekly-backup-v2.9.19.sh"
MIAC_INSTALLER="$SITE_ROOT/install-miac-seed-v2.9.19.sh"
...
if [[ -n "$PROBLEM" ]]; then
  ...
  if [[ "$TOKEN_RC" -ne 0 ]]; then
    ...
    {
      printf '%s\n' "$KEEP"
      # v2.9.19: do not duplicate the exact environment lines installed by earlier P2K CRON installers.
      printf '%s\n' "$KEEP" | grep -qx 'SHELL=/bin/bash' || printf '%s\n' 'SHELL=/bin/bash'
      printf '%s\n' "$KEEP" | grep -qx 'PATH=/usr/local/bin:/usr/bin:/bin' || printf '%s\n' 'PATH=/usr/local/bin:/usr/bin:/bin'
      printf '%s\n' '' '# BEGIN PROMOTE TO KING v2.9.19'
