#!/usr/bin/env bash
# Promote to King v2.9.18 -> v2.9.18 verified incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.17_to_v2.9.18_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.17_to_v2.9.18.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.17-to-v2.9.18.tar.gz"
RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.17-to-v2.9.18.tar.gz"
STAGE="$SITE_ROOT/.p2k-v2917-stage-$STAMP"
WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v2917-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.18" ]]; then
  echo "Promote to King is already VERSION 2.9.18; no files were changed."
  rm -rf "$WORK_TMP" 2>/dev/null || true
  exit 0
fi
[[ "$CURRENT" != "2.9.17" ]] && PROBLEM="Expected verified production VERSION 2.9.17, found '$CURRENT'."
...
printf '%s\n' "===== P2K v2.9.17 -> v2.9.18 VERIFIED INCREMENTAL UPDATE =====" "Root:    $SITE_ROOT" "Archive: $ARCHIVE" "Backup:  compact archives under $BACKUP_ROOT"
...
"$PHP_BIN" -r '
  ...
  if(!is_array($m)||($m["from"]??"")!=="2.9.17"||($m["to"]??"")!=="2.9.18"||!is_array($m["files"]??null)||!is_array($m["removed"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.17-to-v2.9.18" || exit 6
...
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.18" ]] || POST_OK=0
if [[ "$POST_OK" -eq 1 ]]; then
  grep -q 'version: "2.9.18"' "$SITE_ROOT/assets/js/site-config.js" || POST_OK=0
  grep -q '"version": "2.9.18"' "$SITE_ROOT/site-manifest.json" || POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  for f in "$SITE_ROOT/cron-dispatch-v2.9.18.sh" "$SITE_ROOT/reset-install-cron-v2.9.18.sh" "$SITE_ROOT/install-oauth-v2.9.18.sh" "$SITE_ROOT/install-miac-seed-v2.9.18.sh" "$SITE_ROOT/weekly-backup-v2.9.18.sh" "$SITE_ROOT/tools/release/p2k-state-backup.sh" "$SITE_ROOT/tools/release/p2k-state-restore.sh"; do [[ -f "$f" ]] && bash -n "$f" || { POST_OK=0; break; }; done
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  echo "===== INSTALL / VERIFY v2.9.18 CRON ====="
  P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" P2K_PRESERVE_SHARED_CONFIG=1 "$SITE_ROOT/reset-install-cron-v2.9.18.sh" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 && -s "$PROTECTED_HASHES" ]]; then
...
if [[ "$POST_OK" -eq 1 ]]; then
  INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true); COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.18.sh' || true); WEEKLY_COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'weekly-backup-v2.9.18.sh' || true)
  [[ "$COUNT" -eq 4 && "$WEEKLY_COUNT" -eq 1 ]] || { echo "CRON verification found $COUNT operational and $WEEKLY_COUNT weekly v2.9.18 entries; expected 4 + 1." >&2; POST_OK=0; }
fi
...
if [[ "$POST_OK" -eq 1 ]]; then
  ...
  echo "SUCCESS: Promote to King v2.9.18 installed from verified v2.9.17."
...
rollback_update
echo "ERROR: v2.9.18 validation failed; release files, mutable state and CRON were rolled back." >&2
exit 10
