# Promote to King v2.7.2 — pause, remove and install CRON tasks

The endpoint paths and production tokens are unchanged. v2.7.2 uses short, durable scheduler ticks and does not require a self-scheduling PHP chain.
...
```sh
crontab -l > "$HOME/p2k-crontab-before-v2.7.2.txt"
crontab -l
```
...
## 5. Install the v2.7.2 schedules

Open `crontab -e` and add one marked block:

```cron
# BEGIN PROMOTE TO KING v2.7.2
