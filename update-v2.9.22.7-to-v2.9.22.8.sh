#!/usr/bin/env bash
# Promote to King v2.9.22.7 -> v2.9.22.8 verified page-load shedding hotfix.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.7_to_v2.9.22.8_INCREMENTAL.zip"; ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.7_to_v2.9.22.8.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}; BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.7-to-v2.9.22.8.tar.gz"; RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.7-to-v2.9.22.8.tar.gz"
STAGE="$SITE_ROOT/.p2k-v29228-stage-$STAMP"; WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v29228-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.8" ]]; then echo "Promote to King is already VERSION 2.9.22.8; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.7" ]]&&PROBLEM="Expected production VERSION 2.9.22.7, found '$CURRENT'."
...
echo "===== P2K v2.9.22.7 -> v2.9.22.8 PAGE-LOAD SHEDDING HOTFIX ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.7"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.8"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.7-to-v2.9.22.8" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.7 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
APPLY_OK=1
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.8" ]]||POST_OK=0
grep -q 'version: "2.9.22.8"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.8"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  ...
  while IFS= read -r page; do
    while IFS= read -r marker; do [[ -z "$marker" || "$marker" == '?v=2.9.22.8' || "$marker" == '&v=2.9.22.8' ]] || { bad_marker="$page:$marker"; break 2; }; done < <(grep -Eo '[?&]v=[^"'"'"'<>[:space:]&]+' "$page" 2>/dev/null || true)
...
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.8 page-load shedding hotfix installed."; echo "Core 16 / Analytics 7 and existing v2.9.22 CRON remain unchanged; no schema migration is required."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
rollback; echo "ERROR: v2.9.22.8 validation failed; release files were rolled back." >&2; exit 10
