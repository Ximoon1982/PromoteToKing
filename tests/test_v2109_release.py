...
def test_release_identity_and_cache_convergence():
    ...
    assert 'version: "2.10.9.1"' in (ROOT/'assets/js/site-config.js').read_text()
    seen=[]; bad=[]
    for p in list(ROOT.glob('*.html'))+list(ROOT.glob('*.htm')):
        ...
        seen.append(p.name)
        if 'assets/js/site-config.js?v=2.10.9.1' not in text: bad.append(p.name)
    assert len(seen)==21
    assert bad==[]
...
def test_runtime_diagnostics_tracks_current_site_config_generation():
    ...
    assert 'runtime-diagnostics.js?v=2.10.9.1' in health
    assert 'InsightsHealth.html?embedded=1&release=2.10.9.1' in (ROOT/'ui-v2.html').read_text()
    assert 'InsightsHealth.html?embedded=1&release=2.10.9.1' in (ROOT/'assets/js/pages/dashboard-v2.js').read_text()
