...
'catalog Green-native facts': 'FROM p2k_g_matches m' in api and 'p2k_tp_match_metadata' not in api and "'source' => 'p2k-core'" in api,
...
'card compact v18 header': '.pc-title{min-width:0;color:var(--pc-gold);font-size:13px' in card_embed and '.pc-links a{display:inline-flex' in card_embed and 'font-size:8px' in card_embed,
...
'card arena meta emphasis': '.pc-arena-meta-line strong{color:var(--pc-text);font-weight:900}' in card_embed and 'class="pc-arena-meta-line"' in card_embed,
'card league 48h parity': 'pc-league-48h' in card_embed and '48*3600000' in card_embed,
'card long-range muted start': '.pc-muted{color:var(--pc-muted)}' in card_embed and '"pc-muted"' in card_embed,
'card rating emphasis': '.pc-match-rating{color:var(--pc-text);font-weight:900}' in card_embed and 'class="pc-match-rating"' in card_embed,
'card post-hydration fallback': 'let settled=chooseFilter(matches)' in card_embed and 'if(settled!==filter)' in card_embed and 'filter=chooseFilter(matches);renderDaily()' in card_embed,
