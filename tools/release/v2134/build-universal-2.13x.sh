...
OUT="${1:-$ROOT/dist/v2.13.4}"
PKG="PromoteToKing_v2.13.4_INCREMENTAL_FROM_2.13.x"
DIR="$OUT/$PKG"
ZIP="$OUT/$PKG.zip"
...
cp "$TEMPLATE" "$DIR/install-promote-to-king-v2.13.4.sh"
chmod +x "$DIR/install-promote-to-king-v2.13.4.sh"

mapfile -t INSTALLER_FILES < <(
  awk '/^FILES=\($/{inside=1;next} inside&&/^\)$/{exit} inside{gsub(/^[[:space:]]+|[[:space:]]+$/,""); if(length) print}' \
    "$DIR/install-promote-to-king-v2.13.4.sh"
)
...
python3 - "$DIR/install-promote-to-king-v2.13.4.sh" "$HEAD" "$CACHE_KEY" <<'PY'
...
Promote to King v2.13.4 cumulative incremental installer
...
Install:
  ...
  unzip -q $PKG.zip
  cd $PKG
  bash install-promote-to-king-v2.13.4.sh /kunden/homepages/43/d141198007/htdocs/PromoteToKing
TXT
...
if grep -q '@@' "$DIR/install-promote-to-king-v2.13.4.sh"; then
  echo 'Unresolved installer token' >&2
  exit 1
fi
bash -n "$DIR/install-promote-to-king-v2.13.4.sh"
