...
BUILD=${1:-"$ROOT/build-v2113"}
PAYLOAD="$BUILD/payload"
INSTALLER="$BUILD/PromoteToKing_v2.11.3_RUNTIME_CONSOLIDATION.run"
LAUNCHER="$BUILD/install-promote-to-king-2.11.3.sh"
...
case "$INSTALLED" in 2.11|2.11.*) ;; *) echo "ERROR: v2.11.3 requires an installed Promote to King 2.11.x tree; found '$INSTALLED'." >&2; exit 2;; esac
echo "Installing Promote to King v2.11.3 runtime ownership consolidation over v$INSTALLED."
...
install -m 0644 "$TMP/payload/V2113_SOURCE_HEAD.txt" "$TARGET/V2113_SOURCE_HEAD.txt"
install -m 0644 "$TMP/payload/STATIC_ASSET_CACHE_KEY.txt" "$TARGET/STATIC_ASSET_CACHE_KEY.txt"
install -m 0644 "$TMP/payload/MANIFEST.sha256" "$TARGET/MANIFEST_v2.11.3.sha256"
...
transaction=0
echo 'Promote to King v2.11.3 runtime ownership consolidation installed.'
echo 'Existing CRON entries were preserved unchanged.'
exit 0
__P2K_V2113_PAYLOAD_BELOW__
INSTALLER_HEAD
...
exec "$HERE/PromoteToKing_v2.11.3_RUNTIME_CONSOLIDATION.run" "$TARGET"
LAUNCHER_BODY
chmod +x "$LAUNCHER"
cat > "$BUILD/INSTALL_v2.11.3.txt" <<'INSTRUCTIONS'
Promote to King v2.11.3 — Runtime Ownership & Job Orchestration Consolidation
...
Direct installer form
  ./PromoteToKing_v2.11.3_RUNTIME_CONSOLIDATION.run /absolute/path/to/promote-to-king

The installer validates the embedded payload and installed VERSION, applies only the
v2.11.3 structural runtime files, preserves existing CRON entries, and rolls back all
...
(cd "$BUILD" && sha256sum -c SHA256SUMS_v2.11.3.txt)
