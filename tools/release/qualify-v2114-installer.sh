...
BUILD=${1:?Usage: qualify-v2114-installer.sh BUILD_DIRECTORY}
...
install_protected_fixtures() {
  ...
  printf 'local-secret=preserve-byte-for-byte\n' > "$target/config/local/v2114-secret.conf"
  printf '{"runtime":"preserve-byte-for-byte"}\n' > "$target/data/qualification/v2114-state.json"
  printf 'persistent-state=preserve-byte-for-byte\n' > "$target/server/team-points/storage/v2114-state.txt"
}

protected_hashes() {
  local target=$1 output=$2
  (cd "$target" && sha256sum \
    config/local/v2114-secret.conf \
    data/qualification/v2114-state.json \
    server/team-points/storage/v2114-state.txt) > "$output"
}
...
for entry in "${BASELINES[@]}"; do
  ...
  (cd "$target" && sha256sum -c MANIFEST_v2.11.4.sha256)
  ...
  python3 "$ROOT/tools/release/static_asset_cache_key.py" verify --root "$BUILD/payload" --version 2.11.4 --source-head "$SOURCE_HEAD" --build-id "$BUILD_ID" \
...
echo 'v2.11.4 installer qualification passed for every supported 2.11.x baseline class.'
