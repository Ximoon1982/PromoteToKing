...
BUILD=${1:?Usage: qualify-v2113-installer.sh BUILD_DIRECTORY}
...
install_protected_fixtures() {
  ...
  printf 'local-secret=preserve-byte-for-byte\n' > "$target/config/local/v2113-secret.conf"
  printf '{"runtime":"preserve-byte-for-byte"}\n' > "$target/data/qualification/v2113-state.json"
  printf 'persistent-state=preserve-byte-for-byte\n' > "$target/server/team-points/storage/v2113-state.txt"
}

protected_hashes() {
  local target=$1 output=$2
  (cd "$target" && sha256sum \
    config/local/v2113-secret.conf \
    data/qualification/v2113-state.json \
    server/team-points/storage/v2113-state.txt) > "$output"
}
...
for entry in "${BASELINES[@]}"; do
  ...
  (cd "$target" && sha256sum -c MANIFEST_v2.11.3.sha256)
...
echo 'v2.11.3 installer qualification passed for every supported 2.11.x baseline class.'
