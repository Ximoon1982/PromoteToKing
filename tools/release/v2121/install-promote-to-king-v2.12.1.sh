...
# Universal, network-independent P2K 2.11.x -> 2.12.1 installer.
...
preflight() {
  ...
  WORK=$(mktemp -d "$TARGET/.p2k-v2121-preflight.XXXXXX") \
    || fatal "cannot create temporary workspace in target"
  [[ -w "$WORK" ]] || fatal "temporary workspace is not writable"
  verify_package
  OLD_VERSION=$(tr -d '\r\n[:space:]' <"$TARGET/VERSION")
  if [[ "$MODE" == install && "$OLD_VERSION" != "2.12.1" ]]; then
    [[ "$OLD_VERSION" == 2.11.* || "$OLD_VERSION" == "2.12.0" ]] || fatal "unsupported installed version: $OLD_VERSION"
    verify_supported_baseline
    check_destination_parents
    check_disk_space
  fi
...
verify_installed() {
  ...
  version=$(tr -d '\r\n[:space:]' <"$TARGET/VERSION")
  [[ "$version" == "2.12.1" ]] || fatal "semantic version is not 2.12.1"
  enumerate_installed_tree "$WORK/installed.files"
  cmp -s "$FILES" "$WORK/installed.files" \
    || fatal "installed immutable path inventory is not the exact v2.12.1 inventory"
  while IFS= read -r path; do
  ...
  grep -Fq 'version: "2.12.1"' "$TARGET/assets/js/site-config.js" || fatal "runtime version mismatch"
  local asset_key
  asset_key=$(awk -F= '$1 == "asset_cache_key" {print substr($0, index($0,"=")+1)}' "$RELEASE_IDENTITY")
  [[ -n "$asset_key" ]] || fatal "release asset cache key is missing"
  grep -Fq "$asset_key" "$TARGET/assets/js/site-config.js" || fatal "site-config asset key missing"
  grep -Fq "$asset_key" "$TARGET/RecruitMatch.html" || fatal "RecruitMatch asset key missing"
  grep -Fq "$asset_key" "$TARGET/assets/js/admin/tool-registry.js" || fatal "Trophy/Recruitment loader asset key missing"
  grep -Fq "$asset_key" "$TARGET/trophies/index.html" || fatal "Trophy standalone asset key missing"
  log "v2.12.1 verification passed ($(wc -l <"$FILES") immutable files; no obsolete managed files)."
}
...
stage_and_backup() {
  STAGE=$(mktemp -d "$TARGET/.p2k-v2121-stage.XXXXXX")
  BACKUP=$(mktemp -d "$TARGET/.p2k-v2121-backup.XXXXXX")
  cp -a "$PAYLOAD/." "$STAGE/"
...
activate_payload() {
  ...
  while IFS= read -r path; do
    ...
    temporary="$TARGET/$(dirname "$path")/.p2k-v2121.$(basename "$path").tmp"
...
main() {
  ...
  if [[ "$OLD_VERSION" == "2.12.1" ]]; then
    verify_installed
    log "v2.12.1 is already installed; no change was required."
    return
  fi
  stage_and_backup
  ...
  verify_cron_unchanged
  TRANSACTION_ACTIVE=0
  log "Upgraded transactionally from $OLD_VERSION to 2.12.1; mutable state and CRON preserved."
}
