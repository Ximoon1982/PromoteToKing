...
final class Repository
{
    ...
    private function clubPointsForecastTo(string $clubSlug, string $today, int $actualPoints, string $endDate, bool $includeDayOfYear = false): array
    {
        ...
        return [
            ...
            'method'=>[
                ...
                'inputs'=>'actual Club Points from the latest three 30-day calendar blocks; near-linear continuation with a capped +/-8% terminal trend bend and +/-10% points projection bands'
            ]
        ];
    }
    ...
    public function publicOpponentProfile(string $clubSlug, string $opponentSlug): array
    {
        ...
        $capCoverageQuery=$this->pdo->prepare("SELECT COUNT(*) eligible,SUM(g.max_rating_state IN ('capped','open')) known,SUM(g.max_rating_state='unknown') unknown,SUM(g.max_rating_state='unavailable') unavailable FROM p2k_tp_match_metadata metadata JOIN p2k_g_matches g ON g.match_id=metadata.match_id LEFT JOIN p2k_tp_match_summaries summaries ON summaries.club_slug=metadata.club_slug AND summaries.match_id=metadata.match_id LEFT JOIN p2k_tp_opponent_aliases aliases ON aliases.club_slug=metadata.club_slug AND aliases.alias_slug=metadata.opponent_slug WHERE metadata.club_slug=? AND metadata.is_void=0 AND COALESCE(aliases.canonical_slug,metadata.opponent_slug)=? AND metadata.status='finished' AND summaries.result IN ('win','draw','loss')");
        $capCoverageQuery->execute([$clubSlug,$canonical]);$capCoverageRow=$capCoverageQuery->fetch(PDO::FETCH_ASSOC)?:[];$capEligible=(int)($capCoverageRow['eligible']??0);$capKnown=(int)($capCoverageRow['known']??0);$maxRatingCoverage=['eligible_finished'=>$capEligible,'known_finished'=>$capKnown,'unknown_finished'=>(int)($capCoverageRow['unknown']??0),'unavailable_finished'=>(int)($capCoverageRow['unavailable']??0),'known_percent'=>$capEligible>0?round(100*$capKnown/$capEligible,1):100.0];
        $maxRatingRates=[];$mrQuery=$this->pdo->prepare("SELECT CASE WHEN g.max_rating_state='open' OR (g.max_rating_state='capped' AND g.max_rating>1800) THEN 'open' WHEN g.max_rating>1600 THEN 'u1800' WHEN g.max_rating>1400 THEN 'u1600' WHEN g.max_rating>1200 THEN 'u1400' WHEN g.max_rating>1000 THEN 'u1200' ELSE 'u1000' END bucket,COUNT(*) finished,SUM(summaries.result='win') wins,SUM(summaries.result='draw') draws,SUM(summaries.result='loss') losses FROM p2k_tp_match_metadata metadata JOIN p2k_g_matches g ON g.match_id=metadata.match_id LEFT JOIN p2k_tp_match_summaries summaries ON summaries.club_slug=metadata.club_slug AND summaries.match_id=metadata.match_id LEFT JOIN p2k_tp_opponent_aliases aliases ON aliases.club_slug=metadata.club_slug AND aliases.alias_slug=metadata.opponent_slug WHERE metadata.club_slug=? AND metadata.is_void=0 AND COALESCE(aliases.canonical_slug,metadata.opponent_slug)=? AND metadata.status='finished' AND summaries.result IN ('win','draw','loss') AND g.max_rating_state IN ('capped','open') GROUP BY bucket");
