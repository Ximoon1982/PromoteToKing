...
try {
    ...
    fwrite(STDOUT,json_encode(['ok'=>true,'version'=>'2.10.9','discovery'=>$discovery,'acquisition'=>$acquisition,'sync'=>$service->status()],JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT).PHP_EOL);
    exit(0);
} catch (ApiException $e) {
    if($e->errorCode==='MCA_SYNC_BUSY'){fwrite(STDOUT,json_encode(['ok'=>true,'busy'=>true,'version'=>'2.10.9']).PHP_EOL);exit(0);}throw $e;
} catch (Throwable $e) {
