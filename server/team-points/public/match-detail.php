...
try {
    Http::method('GET');
    $matchId=(int)($_GET['match_id']??$_GET['match']??0);
    ...
    $entry=$cache->remember('match-detail|'.$club.'|'.$matchId.'|'.$generation,120,static fn()=>['ok'=>true,'meta'=>$repository->publicInsightsMeta($club),'match'=>$repository->publicMatchDetail($club,$matchId)],600);
...
catch (Throwable $e) { error_log('P2K Match detail: '.$e); Http::json(['ok'=>false,'error'=>['code'=>'SERVER_ERROR','message'=>'Match detail is temporarily unavailable.']],500); }
