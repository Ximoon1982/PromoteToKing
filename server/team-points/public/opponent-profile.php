...
try {
    Http::method('GET');
    $slug=trim((string)($_GET['slug']??$_GET['opponent']??''));
    ...
    $entry=$cache->remember('opponent-profile|'.$club.'|'.strtolower($slug).'|'.$generation,120,static fn()=>['ok'=>true,'meta'=>$repository->publicInsightsMeta($club),'opponent'=>$repository->publicOpponentProfile($club,$slug)],600);
...
catch (Throwable $e) { error_log('P2K Opponent profile: '.$e); Http::json(['ok'=>false,'error'=>['code'=>'SERVER_ERROR','message'=>'Opponent profile is temporarily unavailable.']],500); }
