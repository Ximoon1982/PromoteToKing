#!/usr/bin/env bash
# Promote to King v2.9.22.1 -> v2.9.22.2 verified OAuth-saturation hotfix updater.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.1_to_v2.9.22.2_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.1_to_v2.9.22.2.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}
BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.1-to-v2.9.22.2.tar.gz"
RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.1-to-v2.9.22.2.tar.gz"
STAGE="$SITE_ROOT/.p2k-v29222-stage-$STAMP"
WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v29222-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.2" ]]; then echo "Promote to King is already VERSION 2.9.22.2; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.1" ]]&&PROBLEM="Expected verified production VERSION 2.9.22.1, found '$CURRENT'."
...
echo "===== P2K v2.9.22.1 -> v2.9.22.2 OAUTH SATURATION HOTFIX ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.1"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.2"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.1-to-v2.9.22.2" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.1 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.2" ]]||POST_OK=0
grep -q 'version: "2.9.22.2"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.2"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then node_bin=$(command -v node||true); [[ -n "$node_bin" ]]&&for f in assets/js/shared/api-client.js assets/js/pages/fresh-points-reconstruction.js assets/js/pages/task-control.js assets/js/site-config.js; do "$node_bin" --check "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
...
if [[ "$POST_OK" -eq 1 && -n "$CURL_BIN" ]]; then http=$($CURL_BIN --silent --location --connect-timeout 8 --max-time 20 --output /dev/null --write-out '%{http_code}' "$BASE_URL/" 2>/dev/null||printf 000); [[ "$http" =~ ^2[0-9][0-9]$ ]]||echo "WARNING: external site probe returned HTTP $http; file-integrity validation remains authoritative for this hotfix." >&2; fi
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.2 OAuth-saturation hotfix installed."; echo "Existing v2.9.22 CRON contract retained unchanged."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
