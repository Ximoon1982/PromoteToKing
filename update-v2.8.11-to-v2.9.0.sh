#!/usr/bin/env bash
# Promote to King v2.8.11 -> v2.9.0 argumentless incremental updater.
...
ARCHIVE_NAME="PromoteToKing_v2.8.11_to_v2.9.0_INCREMENTAL.zip"
ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.8.11_to_v2.9.0.json"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$SITE_ROOT/_backup/${STAMP}_v2.8.11_to_v2.9.0"
STAGE="$SITE_ROOT/.p2k-v290-stage-$STAMP"
...
[[ "$CURRENT" != "2.8.11" && "$CURRENT" != "2.9.0" ]] && PROBLEM="Expected production VERSION 2.8.11 (or already-updated 2.9.0), found '$CURRENT'."
...
if [[ -n "$PROBLEM" ]]; then
  ...
  echo "===== P2K v2.8.11 -> v2.9.0 INCREMENTAL UPDATE ====="
  echo "Root:     $SITE_ROOT"
  ...
  if [[ "$EXTRACT_OK" -ne 1 || ! -f "$STAGE/$MANIFEST_NAME" ]]; then
    ...
    "$PHP_BIN" -r '
      ...
      if(!is_array($m)||($m["from"]??"")!=="2.8.11"||($m["to"]??"")!=="2.9.0"||!is_array($m["files"]??null)){fwrite(STDERR,"Invalid incremental manifest.\n");exit(2);}
    ...
    if [[ "$VERIFY_RC" -ne 0 ]]; then
      ...
      POST_OK=$APPLY_OK
      if [[ "$POST_OK" -eq 1 && "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" != "2.9.0" ]]; then echo "ERROR: VERSION validation failed." >&2; POST_OK=0; fi
      if [[ "$POST_OK" -eq 1 ]]; then bash -n "$SITE_ROOT/cron-dispatch-v2.9.0.sh" && bash -n "$SITE_ROOT/reset-install-cron-v2.9.0.sh" || POST_OK=0; fi
      if [[ "$POST_OK" -eq 1 ]]; then
        for f in "$SITE_ROOT/server/shared/TrafficAnalytics.php" "$SITE_ROOT/server/team-points/src/Worker.php" "$SITE_ROOT/server/team-points/src/AchievementCatalog.php"; do "$PHP_BIN" -l "$f" >/dev/null || POST_OK=0; done
      fi
      ...
      if [[ "$POST_OK" -eq 1 ]]; then
        echo "===== INSTALL / VERIFY v2.9.0 CRON ====="
        P2K_SITE_ROOT="$SITE_ROOT" P2K_PHP_CLI="$PHP_BIN" "$SITE_ROOT/reset-install-cron-v2.9.0.sh"
        INSTALLED=$($CRONTAB_BIN -l 2>/dev/null || true)
        COUNT=$(printf '%s\n' "$INSTALLED" | grep -c 'cron-dispatch-v2.9.0.sh' || true)
        [[ "$COUNT" -eq 4 ]] || { echo "ERROR: v2.9.0 CRON verification found $COUNT entries." >&2; POST_OK=0; }
      fi

      if [[ "$POST_OK" -eq 1 ]]; then
        echo "SUCCESS: Promote to King v2.9.0 installed."
        echo "Backup: $BACKUP"
      else
        rollback_update
        echo "ERROR: v2.9.0 update failed validation and was rolled back where feasible." >&2
      fi
    fi
  fi
  rm -rf "$STAGE" 2>/dev/null || true
