#!/usr/bin/env bash
# Promote to King v2.9.22.8 -> v2.9.22.9 verified reconciliation + server headroom hotfix.
...
ARCHIVE_NAME="PromoteToKing_v2.9.22.8_to_v2.9.22.9_INCREMENTAL.zip"; ARCHIVE="$SITE_ROOT/$ARCHIVE_NAME"
MANIFEST_NAME="INCREMENTAL_MANIFEST_v2.9.22.8_to_v2.9.22.9.json"
STAMP=${P2K_BACKUP_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}; BACKUP_ROOT="$SITE_ROOT/_backup"
STATE_ARCHIVE="$BACKUP_ROOT/${STAMP}_state-v2.9.22.8-to-v2.9.22.9.tar.gz"; RELEASE_ARCHIVE="$BACKUP_ROOT/${STAMP}_release-files-v2.9.22.8-to-v2.9.22.9.tar.gz"
STAGE="$SITE_ROOT/.p2k-v29229-stage-$STAMP"; WORK_TMP=$(mktemp -d "${TMPDIR:-/tmp}/p2k-v29229-update.XXXXXX")
...
if [[ "$CURRENT" == "2.9.22.9" ]]; then echo "Promote to King is already VERSION 2.9.22.9; no files were changed."; rm -rf "$WORK_TMP"; exit 0; fi
[[ "$CURRENT" != "2.9.22.8" ]]&&PROBLEM="Expected production VERSION 2.9.22.8, found '$CURRENT'."
...
echo "===== P2K v2.9.22.8 -> v2.9.22.9 RECONCILIATION + SERVER HEADROOM HOTFIX ====="
"$UNZIP_BIN" -q "$ARCHIVE" -d "$STAGE" || exit 3
[[ -f "$STAGE/$MANIFEST_NAME" ]]||{ echo "ERROR: incremental manifest missing." >&2; exit 4; }
grep -Eq '"from"[[:space:]]*:[[:space:]]*"2\.9\.22\.8"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest source version." >&2; exit 5; }
grep -Eq '"to"[[:space:]]*:[[:space:]]*"2\.9\.22\.9"' "$STAGE/$MANIFEST_NAME" || { echo "ERROR: invalid manifest target version." >&2; exit 5; }
...
P2K_BACKUP_STAMP="$STAMP" P2K_BACKUP_RETENTION="${P2K_BACKUP_RETENTION:-3}" "$STAGE/tools/release/p2k-state-backup.sh" "$SITE_ROOT" "state-v2.9.22.8-to-v2.9.22.9" || exit 6
...
rollback(){ echo "ROLLBACK: restoring v2.9.22.8 release files." >&2; rm -rf "$ROLLBACK_RESTORE"; mkdir -p "$ROLLBACK_RESTORE"; "$TAR_BIN" -C "$ROLLBACK_RESTORE" -xzf "$RELEASE_ARCHIVE" 2>/dev/null||true; [[ -f "$ROLLBACK_RESTORE/new-files.txt" ]]&&while IFS= read -r rel; do [[ -n "$rel" ]]&&rm -f "$SITE_ROOT/$rel"; done < "$ROLLBACK_RESTORE/new-files.txt"; if [[ -d "$ROLLBACK_RESTORE/files" ]]; then (cd "$ROLLBACK_RESTORE/files"&&find . -type f -print0)|while IFS= read -r -d '' rel; do rel=${rel#./}; mkdir -p "$SITE_ROOT/$(dirname "$rel")"; cp -p "$ROLLBACK_RESTORE/files/$rel" "$SITE_ROOT/$rel"||true; done; fi; }
APPLY_OK=1
...
POST_OK=$APPLY_OK
[[ "$(tr -d '\r\n ' < "$SITE_ROOT/VERSION" 2>/dev/null)" == "2.9.22.9" ]]||POST_OK=0
grep -q 'version: "2.9.22.9"' "$SITE_ROOT/assets/js/site-config.js"||POST_OK=0; grep -q '"version": "2.9.22.9"' "$SITE_ROOT/site-manifest.json"||POST_OK=0
...
if [[ "$POST_OK" -eq 1 ]]; then
  grep -Fq 'snapshot/status must stay metadata-only' "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq "'review'=>null" "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq 'page bootstrap is status-only' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  grep -Fq 'automatic reconciliation polling removed' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  ! grep -Fq 'scheduleReconciliationRefresh(' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
  ! grep -Fq 'window.P2K_FRESH_POINTS_RECONSTRUCTION=api;init();' "$SITE_ROOT/assets/js/pages/fresh-points-reconstruction.js" || POST_OK=0
  grep -Fq 'await window.P2K_FRESH_POINTS_RECONSTRUCTION?.init?.();' "$SITE_ROOT/assets/js/pages/task-control.js" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then
  bad_marker=""
  while IFS= read -r page; do
    while IFS= read -r marker; do [[ -z "$marker" || "$marker" == '?v=2.9.22.9' || "$marker" == '&v=2.9.22.9' ]] || { bad_marker="$page:$marker"; break 2; }; done < <(grep -Eo '[?&]v=[^"'"'"'<>[:space:]&]+' "$page" 2>/dev/null || true)
...
if [[ "$POST_OK" -eq 1 ]]; then
  grep -Fq 'const RUNTIME_VERSION = "2.9.22.9";' "$SITE_ROOT/assets/js/shared/api-client.js" || POST_OK=0
  grep -Fq 'const OAUTH_BACKGROUND_MAX_POSTS = 2;' "$SITE_ROOT/assets/js/shared/api-client.js" || POST_OK=0
  grep -Fq 'serverForegroundPressure()' "$SITE_ROOT/assets/js/shared/api-client.js" || POST_OK=0
  grep -Fq 'P2K_SERVER_FOREGROUND_REQUESTS' "$SITE_ROOT/assets/js/shared/team-points-client.js" || POST_OK=0
  grep -Fq "return 'final_result_mismatch'" "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq "return 'points_mismatch'" "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq 'positive_delta' "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq 'negative_delta' "$SITE_ROOT/server/team-points/src/FreshPointsReconstruction.php" || POST_OK=0
  grep -Fq 'reconstructionClubDeltaFilter' "$SITE_ROOT/TaskControl.html" || POST_OK=0
  grep -Fq 'data-reconciliation-sort="club" data-sort-key="point_delta"' "$SITE_ROOT/TaskControl.html" || POST_OK=0
fi
if [[ "$POST_OK" -eq 1 ]]; then node_bin=$(command -v node||true); [[ -n "$node_bin" ]]&&for f in assets/js/site-config.js assets/js/pages/task-control.js assets/js/pages/fresh-points-reconstruction.js assets/js/shared/api-client.js assets/js/shared/team-points-client.js; do "$node_bin" --check "$SITE_ROOT/$f" >/dev/null||POST_OK=0; done; fi
...
if [[ "$POST_OK" -eq 1 ]]; then echo "SUCCESS: Promote to King v2.9.22.9 reconciliation + server headroom hotfix installed."; echo "Core 16 / Analytics 7 and existing v2.9.22 CRON remain unchanged; no schema migration is required."; echo "Rollback: $RELEASE_ARCHIVE"; exit 0; fi
rollback; echo "ERROR: v2.9.22.9 validation failed; release files were rolled back." >&2; exit 10
